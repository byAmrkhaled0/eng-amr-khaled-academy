...
for (const name of htmlFiles) {
  ...
  if (!source.includes('assets/v61-design.css?v=63.0.1')) failures.push(`${name} does not load the V63 design`);
...
if (!login.includes('v60-payments.js?v=63.0.1')) failures.push('Payment UI is not in the built admin page');
if (!login.includes('v60-admin-workflow.js?v=63.0.1')) failures.push('Exam and assignment UI is not in the built admin page');
if (!worker.includes('technominds-v63-0-1-production-fixed')) failures.push('Built service worker cache version is stale');
