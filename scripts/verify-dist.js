...
const failures = [];
const required = ['index.html','404.html','teacher-login.html','service-worker.js','assets/app.js','assets/admin.js','assets/v60-payments.js','assets/v60-admin-workflow.js','assets/v60-technominds.css','assets/v61-design.css','assets/curriculum-admin.js','assets/curriculum-student.js'];
...
for (const name of htmlFiles) {
  ...
  if (!source.includes('assets/v61-design.css?v=63.0.4')) failures.push(`${name} does not load the V63 design`);
...
if (!login.includes('v60-payments.js?v=63.0.4')) failures.push('Payment UI is not in the built admin page');
if (!login.includes('v60-admin-workflow.js?v=63.0.4')) failures.push('Exam and assignment UI is not in the built admin page');
if (!worker.includes('technominds-v63-0-4-production-fixed')) failures.push('Built service worker cache version is stale');
...
console.log(`✓ dist verified (${htmlFiles.length} HTML pages, V63 secure assessment workflow and V61 curriculum assets, no backend/.env)`);
