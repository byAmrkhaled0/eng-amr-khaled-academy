...
for (const name of htmlFiles) {
  ...
  if (!source.includes('assets/v61-design.css?v=62.7.0')) failures.push(`${name} does not load the V62 design`);
...
if (!login.includes('v60-payments.js?v=62.7.0')) failures.push('Payment UI is not in the built admin page');
if (!login.includes('v60-admin-workflow.js?v=62.7.0')) failures.push('Exam and assignment UI is not in the built admin page');
if (!worker.includes('technominds-v62-7-0-performance-release')) failures.push('Built service worker cache version is stale');
...
console.log(`✓ dist verified (${htmlFiles.length} HTML pages, V62 academic workflow and V61 curriculum assets, no backend/.env)`);
