...
test('refresh, admin curriculum and requested visual fixes are part of the release', () => {
  ...
  assert.match(app, /MF_ASSET_VERSION = '63\.0\.1'/);
  assert.match(app, /loadStudentForPortal\(code,\{force:true\}\)/);
  assert.match(worker, /technominds-v63-0-1-production-fixed/);
  ...
  assert.match(login, /assets\/curriculum-admin\.js\?v=63\.0\.1/);
