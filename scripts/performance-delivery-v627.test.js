...
test('refresh, admin curriculum and requested visual fixes are part of the release', () => {
  ...
  assert.match(app, /MF_ASSET_VERSION = '62\.7\.0'/);
  assert.match(app, /loadStudentForPortal\(code,\{force:true\}\)/);
  assert.match(worker, /technominds-v62-7-0-performance-release/);
  ...
  assert.match(login, /assets\/curriculum-admin\.js\?v=62\.7\.0/);
