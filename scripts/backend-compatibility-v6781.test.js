...
test('portal compatibility follows the API schema instead of the deployment release', () => {
  ...
  assert.match(sync, /const FRONTEND_VERSION='70\.0\.2'/);
...
test('every secure portal page requests a fresh firebase sync bundle', () => {
  ...
  for (const page of pages) {
    assert.match(read(page), /assets\/firebase-sync\.js\?v=70\.0\.2/, `${page} must bypass the stale sync bundle`);
  }
});
...
test('release and service-worker cache versions are synchronized', () => {
  ...
  assert.equal(frontend.version, '70.0.2');
  assert.equal(backend.version, frontend.version);
  assert.match(worker, /technominds-v70-0-2-complete-report/);
  assert.match(worker, /ASSET_VERSION = "70\.0\.2"/);
});
