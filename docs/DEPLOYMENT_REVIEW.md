# تحديث حالة النشر — 13 سبتمبر 2026

PR18 مدموج، وmain المفحوص3eb73c6. القياس الحالي42/42 GET رجع200 على الاستضافتين، والصحة67.8.5. هذا يصحح القياس التاريخي أدناه ولا يثبت نشر تعديلات هذه الجلسة. التعديلات الحالية على فرع مراجعة محلي ولم تُنشر.

يلزم بعد مراجعة staging نشر Storage rules والفهارس ثم Functions ثم البناء الجديد. تضاف حماية PDF وحقول lectureSource/lectureTitle وcursor المركب ومنطق النتائج المشترك. أصول المتصفح لها بصمة محتوى rev حتى مع بقاء الإصدار67.8.5. انشر حزمة Functions كاملة لتغطية استدعاءات المنطق المشترك؛ عند اقتراح حذف وظائف قديمة اخترNo إلى أن تُراجع تبعياتها. لا تعديل IAM أو minInstances من هذه الدفعة.

اختُبر المصدر والبناء على Node22.23.2، و15 حالة تكامل على Java21 ومحاكيات demo فقط. أمر check-deployment.ps1 لا يقبل ExpectedVersion؛ يقرأه من functions/package.json. تشغيله لا يثبت الرحلات المحمية، ولم يُشغّل PowerShell هنا.
...
6. انشر الواجهة إلى الاستضافتين المطلوبتين. `check-deployment.ps1 -BaseUrl <staging-url>` يفحص GET الصحة وأصول الإصدار. PowerShell غير متاح هنا، فلم يُشغّل السكربت فعليًا في هذه البيئة. أعلام الخدمات ليست اختبارًا للحجز أو السداد أو الامتحان.
