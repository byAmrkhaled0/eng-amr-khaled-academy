...
exports.createPaymentTransaction = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const requestedCourse = text(body.course || student.grade, 100);
  const course = sameAcademicValue(requestedCourse, student.grade) ? text(canonicalAcademicLabel(student.grade),100) : text(canonicalAcademicLabel(requestedCourse),100);
  ...
  await db.runTransaction(async tx => {
    ...
    if (existingTransaction.exists) {
      ...
      result = { id: existingTransaction.id, duplicate: true, transactionStatus:'active', periodId: existing.periodId, studentCode: existing.studentCode, amount: money(existing.amount), expectedAmount:duplicateExpected, paidAmount:duplicatePaid, remainingAmount:duplicateRemaining, status:paymentStatus(duplicateExpected,duplicatePaid), lastPaymentDate:summary.lastPaymentDate || existing.paymentDate };
      return;
    }
    ...
    result = { id: transactionRef.id, duplicate: false, transactionStatus:'active', periodId, studentCode, amount, expectedAmount: summary.expectedAmount, paidAmount: summary.paidAmount, remainingAmount: summary.remainingAmount, status: summary.status, lastPaymentDate:summary.lastPaymentDate };
  });
  if (!result.duplicate) paymentDashboardCache.clear();
  return result;
});
...
exports.editPaymentTransaction = onCall(CALLABLE_OPTIONS, async request => {
  ...
  await db.runTransaction(async tx => {
    ...
    if ((request.data?.month && request.data.month !== original.month) ||
        (request.data?.academicYear && request.data.academicYear !== original.academicYear) ||
        (request.data?.course && !sameAcademicValue(request.data.course, original.course)))
      throw new HttpsError('invalid-argument', 'نقل الدفعة إلى شهر أو كورس آخر غير مدعوم في محرر العملية الحالي.');
    ...
    if (!paidOn || !paymentMethod) throw new HttpsError('invalid-argument', 'تاريخ الدفع أو طريقة الدفع غير صالحين.');
    let lastPaymentDate = paidOn > (current.lastPaymentDate || '') ? paidOn : current.lastPaymentDate || paidOn;
    if (original.paymentDate === current.lastPaymentDate && paidOn < original.paymentDate) {
      const active = await tx.get(db.collection('payment_transactions').where('periodId', '==', original.periodId));
      lastPaymentDate = active.docs.filter(doc => doc.id !== transactionId && doc.data().status === 'active')
        .reduce((latest, doc) => doc.data().paymentDate > latest ? doc.data().paymentDate : latest, paidOn);
    }
    const summary = { ...current, ...totals, lastPaymentDate, updatedAt: FieldValue.serverTimestamp() };
    ...
    result = { id: transactionId, amount: newAmount, expectedAmount: summary.expectedAmount, paidAmount: summary.paidAmount, remainingAmount: summary.remainingAmount, status: summary.status, lastPaymentDate:summary.lastPaymentDate };
  });
...
exports.cancelPaymentTransaction = onCall(CALLABLE_OPTIONS, async request => {
  ...
  await db.runTransaction(async tx => {
    ...
    if (original.status === 'cancelled') {
      const existingSummary=await tx.get(db.collection('monthly_payments').doc(original.periodId));
      const summary=existingSummary.exists?existingSummary.data():{};
      const expectedAmount=money(summary.expectedAmount||original.expectedAmount),paidAmount=money(summary.paidAmount);
      result={id:transactionId,cancelled:true,alreadyCancelled:true,expectedAmount,paidAmount,remainingAmount:money(Math.max(0,expectedAmount-paidAmount)),status:paymentStatus(expectedAmount,paidAmount),lastPaymentDate:summary.lastPaymentDate||''};
      return;
    }
    const summaryRef = db.collection('monthly_payments').doc(original.periodId);
    const [summarySnap, studentSnap] = await Promise.all([tx.get(summaryRef), tx.get(db.collection('students').doc(cleanDocId(original.studentCode)))]);
    ...
    let lastPaymentDate = totals.paidAmount > 0 ? current.lastPaymentDate || '' : '';
    if (lastPaymentDate && original.paymentDate === lastPaymentDate) {
      const active = await tx.get(db.collection('payment_transactions').where('periodId', '==', original.periodId));
      lastPaymentDate = active.docs.filter(doc => doc.id !== transactionId && doc.data().status === 'active')
        .reduce((latest, doc) => doc.data().paymentDate > latest ? doc.data().paymentDate : latest, '');
    }
    const summary = { ...current, ...totals, activeTransactionCount: Math.max(0, Number(current.activeTransactionCount || 1) - 1), lastPaymentDate, updatedAt: FieldValue.serverTimestamp() };
    tx.set(transactionRef, {
      status: 'cancelled',
    ...
    result = { id: transactionId, cancelled: true, expectedAmount: summary.expectedAmount, paidAmount: summary.paidAmount, remainingAmount: summary.remainingAmount, status: summary.status, lastPaymentDate:summary.lastPaymentDate };
  });
  if (!result.alreadyCancelled) paymentDashboardCache.clear();
  return result;
});
...
function publicMotivationTransaction(id, row = {}) {
  return {
    ...
    source: row.source || (row.reversalOf ? 'manual_reversal' : 'manual'),
    referenceId: text(row.referenceId||row.reversalOf, 100),
    recordedByRole: text(row.recordedByRole, 40),
...
exports.addStudentMotivationPoints = onCall(CALLABLE_OPTIONS, async request => {
  ...
  if (!validPaymentAcademicYear(academicYear) || !PAYMENT_MONTH_NAMES.includes(month)) throw new HttpsError('invalid-argument', 'حدد العام الدراسي والشهر.');
  ...
  await db.runTransaction(async tx => {
    ...
    if (existing.exists) {
      ...
      if (row.studentCode !== studentCode || row.academicYear !== academicYear || row.month !== month || Number(row.points) !== points || row.reason !== reason || String(row.notes||'') !== notes)
        throw new HttpsError('already-exists', 'معرّف الطلب مستخدم لحركة تحفيز أخرى.');
      result = { duplicate: true, transaction: {...publicMotivationTransaction(existing.id, row),recordedByUid:text(row.recordedByUid,100)}, totalPoints: Number(summarySnap.data()?.totalPoints || 0) };return;
    }
    ...
    result = { duplicate: false, transaction: { ...publicMotivationTransaction(transactionRef.id, transaction), recordedByUid: staff.uid, createdAt: new Date().toISOString() }, totalPoints };
  });
  if (!result?.duplicate) await markLeaderboardDirty('motivation-points');
  await refreshFrozenMotivationMonth(academicYear, month);
  return result;
});
...
exports.getStudentMotivationAdmin = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const summaries = summariesSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })).sort((a,b)=>firestoreMillis(b.updatedAt)-firestoreMillis(a.updatedAt)).map(row => ({ id: text(row.id, 100), academicYear: text(row.academicYear, 30), month: text(row.month, 40), totalPoints: Number(row.totalPoints || 0), paperExamPoints:Number(row.paperExamPoints||0), transactionCount: Number(row.transactionCount || 0), lastReason: text(row.lastReason, 160) }));
  const transactions = transactionsSnap.docs.map(doc => ({...publicMotivationTransaction(doc.id, doc.data()),recordedByUid:text(doc.data().recordedByUid,100),recordedByEmail:text(doc.data().recordedByEmail,160)})).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
...
exports.reverseStudentMotivationTransaction = onCall(CALLABLE_OPTIONS, async request => {
  ...
  await db.runTransaction(async tx => {
    ...
    if (original.reversalOf) throw new HttpsError('failed-precondition', 'لا يمكن عكس حركة التراجع نفسها.');
    if(original.source==='paper_exam')throw new HttpsError('failed-precondition','درجة الامتحان الورقي تُعدّل من شاشة درجات الامتحان فقط.');
    if (original.reversed === true) { result = { duplicate: true, transactionId: original.reversedByTransactionId || '', academicYear: original.academicYear, month: original.month }; return; }
    const existing = await tx.get(reversalRef);
    if (existing.exists) {
      if (existing.data().reversalOf !== transactionId) throw new HttpsError('already-exists', 'معرّف الطلب مستخدم لتراجع مختلف.');
      result = { duplicate: true, transactionId: existing.id, academicYear: original.academicYear, month: original.month }; return;
    }
    ...
    result = { duplicate: false, transactionId: reversalRef.id, totalPoints, academicYear: original.academicYear, month: original.month };
  });
  if (!result?.duplicate) await markLeaderboardDirty('motivation-reversal');
  await refreshFrozenMotivationMonth(result.academicYear, result.month);
  return result;
});
...
exports.getMotivationLeaderboardAdmin = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const period=leaderboardPeriod(academicYear,month),previous=previousPeriodFor(period);
  let rows=await leaderboardRowsForPeriod(academicYear,month),previousRows=await leaderboardRowsForPeriod(previous.academicYear,previous.monthName);
  const filter=row=>scheduleId?String(row.scheduleId||'')===scheduleId:grade?canonicalLeaderboardGrade(row.grade)===canonicalLeaderboardGrade(grade):true;
  rows=enrichLeaderboardRows(rows.filter(filter),previousRows.filter(filter));
  const visible=request.data?.alertsOnly===true?rows.filter(row=>row.maxAbsenceStreak>=2||row.missingHomeworkCount>0||row.missedExamCount>0):rows.slice(0,100);
  return visible.map(row=>({...row,studentName:row.name,name:undefined,totalPoints:Number(row.motivationPoints||0)}));
});

exports.getMotivationSettingsAdmin = onCall(CALLABLE_OPTIONS, async request => {
  await requireStaff(request,['admin']);
  const snap=await db.collection('settings').doc('platform').get();
  return motivationConfigFromSettings(snap.exists?snap.data():{});
});

exports.saveMotivationSettingsAdmin = onCall(CALLABLE_OPTIONS, async request => {
  const staff=await requireStaff(request,['admin']);
  const motivationConfig=motivationConfigFromSettings({motivationConfig:request.data||{}});
  await db.collection('settings').doc('platform').set({motivationConfig,motivationConfigUpdatedAt:FieldValue.serverTimestamp(),motivationConfigUpdatedBy:staff.uid},{merge:true});
  await markLeaderboardDirty('motivation-settings');
  return motivationConfig;
});

async function archiveLeaderboardPeriod(period){
  const rows=await leaderboardRowsForPeriod(period.academicYear,period.monthName,{recalculate:true}),batch=db.batch();
  const source=await leaderboardStateRef.get(),sourceVersion=Number(source.data()?.version||0);
  ACADEMIC_GRADES.forEach(grade=>{const gradeRows=enrichLeaderboardRows(rows.filter(row=>canonicalLeaderboardGrade(row.grade)===canonicalLeaderboardGrade(grade))),id=`${period.monthKey}_${hash(grade).slice(0,12)}`;batch.set(db.collection('leaderboard_archives').doc(id),{academicYear:period.academicYear,month:period.monthName,monthKey:period.monthKey,grade,rows:gradeRows,studentCount:gradeRows.length,sourceVersion,attendancePolicyVersion:'monthly-v12-scheduled-session-attendance',lockedAt:FieldValue.serverTimestamp()},{merge:true});});
  await batch.commit();return {ok:true,monthKey:period.monthKey,studentCount:rows.length};
}

async function refreshFrozenMotivationMonth(academicYear,month){
  const period=leaderboardPeriod(academicYear,month);
  if(period.monthKey===leaderboardPeriod().monthKey)return;
  const archived=await db.collection('leaderboard_archives').where('monthKey','==',period.monthKey).limit(1).get();
  if(!archived.empty)await archiveLeaderboardPeriod(period);
}

exports.freezeLeaderboardMonthAdmin = onCall({ ...CALLABLE_OPTIONS, timeoutSeconds:120, memory:'512MiB' },async request=>{await requireStaff(request,['admin']);return archiveLeaderboardPeriod(leaderboardPeriod(text(request.data?.academicYear,30),text(request.data?.month,40)));});
exports.freezeMonthlyLeaderboard = onSchedule({region:'europe-west1',schedule:'10 0 1 * *',timeZone:'Africa/Cairo',timeoutSeconds:180,memory:'512MiB'},async()=>archiveLeaderboardPeriod(previousLeaderboardPeriod()));
...
function portalResponse(data, attempts, records = {}) {
  ...
  const paymentPeriod = periodFromMonthKey(cairoDateKey().slice(0,7));
  return {
    ...apiMetadata(),
    ...
    paid: paymentPeriodStatus(records.monthlyPayments, paymentPeriod.academicYear, paymentPeriod.monthName) === 'paid',
    ...
    monthlyPayments: (Array.isArray(records.monthlyPayments) ? records.monthlyPayments : (Array.isArray(data.monthlyPayments) ? data.monthlyPayments : [])).slice(-120).map(row => ({
      ...
      status: paymentStatus(row.expectedAmount, row.paidAmount),
...
async function attemptSummaries(studentCode) {
  ...
  const [canonical,sub] = await Promise.all([
    db.collection('exam_attempts').where('studentCode','==',studentCode).limit(180).get(),
    parentRef.collection('attempts').orderBy('submittedAt', 'desc').limit(120).get().catch(() => null)
  ]);
  const summaries=(sub?.docs||[]).map(doc=>({id:doc.id,...doc.data()})),summaryById=new Map(summaries.map(row=>[row.id,row]));
  const official=(canonical?.docs||[]).map(doc=>{const row={id:doc.id,...doc.data()},mirror=summaryById.get(doc.id)||{};return {...mirror,...row,review:Array.isArray(row.answers)?row.answers.map(answer=>({question:answer.question,type:answer.type,answer:answer.answer,mark:answer.mark,awardedMark:answer.awardedMark,correct:answer.correct,...(mirror.answersRevealed===true?{correctAnswer:answer.correctAnswer}:{})})):mirror.review||[],answersRevealed:mirror.answersRevealed===true};});
  let attempts=[...official,...summaries.filter(row=>!official.some(item=>item.id===row.id||String(item.examId)===String(row.examId)))];
  if (!summaries.length) {
    const legacy = await parentRef.get();
    if(legacy.exists&&Array.isArray(legacy.data().attempts))attempts.push(...legacy.data().attempts.filter(row=>!official.some(item=>String(item.examId)===String(row.examId))));
  }
  return attempts.sort((a,b)=>String(b.submittedAt||'').localeCompare(String(a.submittedAt||''))).slice(0,120).map(a => ({
    id: text(a.id, 120),
...
const ACADEMIC_GRADE_QUERY_VALUES = Object.freeze({
  ...
  'أساسيات برمجة': ['أساسيات برمجة','اساسيات برمجة','أساسيات برمجه','اساسيات برمجه','أساسيات Python','اساسيات Python','تطبيقات ومراجعة','مبتدئين برمجة','مبتدئين برمجه','مبتدئين']
});
...
function contentAvailableAfterStudentJoined(item={},student={},atTime=Date.now()){
  ...
  const today=cairoDateKey(new Date(atTime)),dueDate=String(item.dueDate||'').trim();
  if(/^\d{4}-\d{2}-\d{2}$/.test(dueDate)&&today<=dueDate&&assignmentIsReleased(item))return true;
  const now=atTime,openAt=scheduledTimeMillis(item.openAt),closeAt=scheduledTimeMillis(item.closeAt);
...
exports.getPortalStudent = onCall(EXAM_ENTRY_OPTIONS, async request => {
  ...
  let monthlyReport=null,monthlyReportError=false;
  if(approved&&mode==='student'){
    try{monthlyReport=await buildStudentMonthlyReport(secureStudent,cairoDateKey(new Date()).slice(0,7));}
    catch(_){monthlyReportError=true;}
  }
  return {
...
exports.searchStudentsAdmin = onCall(CALLABLE_OPTIONS, async request => {
  ...
  snapshots.filter(Boolean).forEach(snap=>snap.docs.forEach(doc=>rows.set(doc.id,{id:doc.id,...doc.data()})));
  return [...rows.values()].filter(row=>(!grade||sameAcademicValue(row.grade,grade))&&(!scheduleId||String(row.scheduleId||row.groupId||'')===scheduleId)).slice(0,50).map(row=>({ studentCode:text(row.studentCode||row.code||row.id,40), studentName:text(row.studentName||row.name,100), parentPhone:digits(row.parentPhone), studentPhone:digits(row.studentPhone), grade:text(canonicalAcademicLabel(row.grade),80), group:text(row.group,100), scheduleId:text(row.scheduleId||row.groupId,100), active:row.active!==false, academicYear:text(row.academicYear,30), term:text(row.term,40) }));
});
...
exports.getStudentAdminProfile = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const [source,notesSnap,ranking,motivationTransactionsSnap,reportDeliveriesSnap]=await Promise.all([
    ...
    db.collection('parent_report_deliveries').where('studentCode','==',studentCode).limit(80).get().catch(()=>null)
  ]);
  ...
  const student={
    ...
    grade:text(canonicalAcademicLabel(found.data.grade),80),
    group:text(found.data.group,100),
    academicYear:text(found.data.academicYear,30),
  ...
  const reportDeliveries=reportDeliveriesSnap?reportDeliveriesSnap.docs.map(doc=>({id:doc.id,...doc.data()})).sort((a,b)=>firestoreMillis(b.createdAt)-firestoreMillis(a.createdAt)):[];
  return { student, period:{academicYear,month,monthKey:periodKey}, monthlyReport, attendance:monthlyReport.attendance.rows, grades:monthlyReport.results.rows, results:monthlyReport.results.rows, examAttempts:monthlyReport.results.rows, homeworks:monthlyReport.homework.rows, lectures:monthlyReport.study.rows, recitations:monthlyReport.practical.rows, monthlyPayments:source.payments, motivationSummaries:source.motivation, motivationTransactions, reportDeliveries, privateNotes:notesSnap?notesSnap.docs.map(doc=>({id:doc.id,...doc.data()})):[] };
});
...
for(const collection of ['assignments','exams','class_sessions','materials','groups']){
  exports[`invalidateReport_${collection}`]=onDocumentWritten({document:`${collection}/{id}`,region:'europe-west1'},async()=>{
    await db.doc('_system/report_content').set({version:FieldValue.increment(1),updatedAt:FieldValue.serverTimestamp()},{merge:true});
    if(collection==='class_sessions'||collection==='groups')await markLeaderboardDirty(`${collection}-updated`);
  });
}
...
function reportIso(value) {
  ...
  return Number.isFinite(date.getTime())?date.toISOString():text(value,60);
}

function reportMonthForRow(row={},preferredFields=[]) {
  for(const field of [...preferredFields,'date','submittedAt','completedAt','lastOpenedAt','createdAt','updatedAt']){
    const key=cairoDateKey(row[field]).slice(0,7);if(key)return key;
  }
  if(row.academicYear&&row.month){try{return leaderboardPeriod(String(row.academicYear),String(row.month)).monthKey;}catch(_){}}
  return '';
}
...
function reportMonthBounds(monthKeys=[]) {
  const keys=[...new Set(monthKeys.filter(key=>/^\d{4}-(0[1-9]|1[0-2])$/.test(String(key))))].sort();
  if(!keys.length)return null;
  const [startYear,startMonth]=keys[0].split('-').map(Number),[endYear,endMonth]=keys.at(-1).split('-').map(Number);
  const start=new Date(Date.UTC(startYear,startMonth-1,1)),end=new Date(Date.UTC(endYear,endMonth,1));
  return {startKey:keys[0]+'-01',endKey:`${end.getUTCFullYear()}-${String(end.getUTCMonth()+1).padStart(2,'0')}-01`,startTimestamp:admin.firestore.Timestamp.fromDate(start),endTimestamp:admin.firestore.Timestamp.fromDate(end)};
}

async function successfulReportSnapshots(promises) {
  const settled=await Promise.allSettled(promises),snapshots=settled.filter(item=>item.status==='fulfilled').map(item=>item.value).filter(Boolean);
  if(!snapshots.length){const failure=settled.find(item=>item.status==='rejected');if(failure)throw failure.reason;}
  return snapshots;
}

async function reportRowsForPeriod(collection,studentCode,{studentFields=['studentCode'],dateField,dateType='timestamp',monthKeys=[]}={}) {
...
async function reportContentForPeriod(collection,dateField,monthKeys=[]) {
  const bounds=reportMonthBounds(monthKeys);if(!bounds)return fetchAllCollectionDocuments(collection);
  const snapshots=await successfulReportSnapshots([[bounds.startTimestamp,bounds.endTimestamp],[bounds.startKey,bounds.endKey]].map(([start,end])=>db.collection(collection).where(dateField,'>=',start).where(dateField,'<',end).get()));
  const rows=new Map();snapshots.filter(Boolean).forEach(snap=>snap.docs.forEach(doc=>rows.set(doc.id,{id:doc.id,...doc.data()})));
  return [...rows.values()];
}

async function reportContentForAnyPeriodField(collection,dateFields=[],monthKeys=[]) {
  const bounds=reportMonthBounds(monthKeys);if(!bounds)return fetchAllCollectionDocuments(collection);
  const queries=dateFields.flatMap(dateField=>[[bounds.startTimestamp,bounds.endTimestamp],[bounds.startKey,bounds.endKey]].map(([start,end])=>db.collection(collection).where(dateField,'>=',start).where(dateField,'<',end).get()));
  const snapshots=await successfulReportSnapshots(queries),rows=new Map();
  snapshots.filter(Boolean).forEach(snap=>snap.docs.forEach(doc=>rows.set(doc.id,{id:doc.id,...doc.data()})));
  return [...rows.values()];
}

async function reportAcademicRowsForPeriods(collection,studentCode,monthKeys=[]) {
  const periods=monthKeys.map(periodFromMonthKey);
  const snapshots=await successfulReportSnapshots(periods.map(period=>db.collection(collection).where('studentCode','==',studentCode).where('academicYear','==',period.academicYear).where('month','==',period.monthName).get()));
  const rows=new Map();snapshots.filter(Boolean).forEach(snap=>snap.docs.forEach(doc=>rows.set(doc.id,{id:doc.id,...doc.data()})));
  return [...rows.values()];
}

async function reportSubcollectionForPeriod(ref,dateField,monthKeys=[]) {
  const bounds=reportMonthBounds(monthKeys);if(!bounds){const snap=await ref.get();return snap.docs.map(doc=>({id:doc.id,...doc.data()}));}
  const snapshots=await successfulReportSnapshots([[bounds.startTimestamp,bounds.endTimestamp],[bounds.startKey,bounds.endKey]].map(([start,end])=>ref.where(dateField,'>=',start).where(dateField,'<',end).get()));
  const rows=new Map();snapshots.filter(Boolean).forEach(snap=>snap.docs.forEach(doc=>rows.set(doc.id,{id:doc.id,...doc.data()})));
  return [...rows.values()];
}
...
async function loadStudentMonthlyReportSource(student,options={}) {
  ...
  const historicalTarget=(row,allowOpenEnrollment=false)=>{let date=cairoDateKey(row.publishAt||row.openAt||row.createdAt);const joined=firestoreMillis(student.acceptedAt||student.activatedAt||student.enrolledAt||student.createdAt),closeAt=scheduledTimeMillis(row.closeAt);if(allowOpenEnrollment&&joined&&closeAt>joined&&date<cairoDateKey(new Date(joined)))date=cairoDateKey(new Date(joined));const membership=membershipAt(student,transfers,date);return membership&&learningTargetMatchesStudent(row,{...student,...membership,groupId:membership.scheduleId});};
  ...
  const groupRows=await reportReferencedDocuments('groups',historicalScheduleIds);
  const groupsBySchedule=new Map(groupRows.map(row=>[String(row.id),row]));
  const sessions=actualSessionsForStudent(student,sessionGroups.flat(),attendance,transfers,groupsBySchedule);
  const started=await reportRowsForPeriod('exam_sessions',studentCode,{dateField:'startedAt',monthKeys});
  const authoritativeExamIds=new Set(attempts.map(row=>String(row.examId||'')).filter(Boolean));
  const examRows=new Map([...started.map(row=>({...row,status:row.status==='submitted'?row.status:'started',examId:row.examId})),...legacyAttempts.filter(row=>!authoritativeExamIds.has(String(row.examId||''))),...attempts].map(row=>[String(row.id),row]));
  ...
  const [assignmentDocs,examDocs,materialDocs]=await Promise.all([
    reportReferencedDocuments('assignments',homeworks.map(row=>row.assignmentId||row.homeworkId||row.assignment?.id||row.assignmentSnapshot?.id),assignmentSeed),
  ...
  const assignments=assignmentDocs.filter(row=>assignmentIsReleased(row)&&historicalTarget(row)&&contentAvailableAfterStudentJoined(row,student));
  const exams=examDocs.filter(row=>{const closeAt=scheduledTimeMillis(row.closeAt);return examIsPublished(row)&&historicalTarget(row,true)&&contentAvailableAfterStudentJoined(row,student,closeAt?closeAt-1:Date.now());});
  const latestRequestedMonth=monthKeys.slice().sort().at(-1)||'',currentMonth=cairoDateKey(new Date()).slice(0,7);
  const crossMonthClosings=[...new Set(exams.filter(row=>monthKeys.includes(reportMonthForRow(row,['openAt','createdAt','closeAt']))).map(row=>cairoDateKey(row.closeAt).slice(0,7)).filter(key=>key>latestRequestedMonth&&key<=currentMonth))];
  const laterExamAttempts=crossMonthClosings.length?await reportRowsForAnyPeriodField('exam_attempts',studentCode,{studentFields:['studentCode','studentId','code'],dateFields:['submittedAt'],monthKeys:crossMonthClosings}):[];
  ...
  progressLectures.forEach(row=>{if(!monthlyProgress.some(item=>String(item.lectureId)===row.id))monthlyProgress.push({lectureId:row.id,...row});});
  return {sessions,sessionsComplete:true,groupsBySchedule,transfers,attendance,grades,homeworks,recitations,payments,motivation,examAttempts:[...new Map([...examRows.values(),...laterExamAttempts].map(row=>[String(row.id),row])).values()],progress:monthlyProgress,assignments,exams,materials};
}
...
function monthlyReportInput(student,source,monthKey) {
  ...
  // A review or edit updates the existing activity; it does not move that
  // activity to the month in which the teacher reviewed it.
  const activityInMonth=(rows,fields)=>rows.filter(row=>reportMonthForRow(row,fields)===monthKey);
  const paymentMonth=row=>{
    if(row.academicYear&&row.month)try{return leaderboardPeriod(String(row.academicYear),String(row.month)).monthKey;}catch(_){}
    return reportMonthForRow(row);
  };
  const period=periodFromMonthKey(monthKey);
  const monthlyHomeworks=activityInMonth(source.homeworks,['submittedAt','date','createdAt','reviewedAt','gradedAt','updatedAt']);
  const requiredAssignments=activityInMonth(source.assignments,['publishAt','createdAt','dueDate']),activityAssignmentIds=new Set(monthlyHomeworks.map(row=>String(row.assignmentId||row.homeworkId||row.assignment?.id||row.assignmentSnapshot?.id)).filter(Boolean));
  ...
  const monthlyAttempts=activityInMonth(source.examAttempts,['submittedAt','date','startedAt','reviewedAt','createdAt']);
  const canonicalExamIds=new Set(source.examAttempts.map(row=>String(row.examId||'')).filter(Boolean));
  const monthlyGrades=activityInMonth(source.grades,['date','submittedAt','createdAt','reviewedAt']).filter(row=>!row.examId||!canonicalExamIds.has(String(row.examId)));
  const otherMonthSubmittedIds=new Set(source.examAttempts.filter(row=>row.submittedAt&&reportMonthForRow(row,['submittedAt','date','startedAt'])!==monthKey&&row.status!=='started'&&row.status!=='absent').map(row=>String(row.examId||'')).filter(Boolean));
  const requiredExams=activityInMonth(source.exams,['openAt','createdAt','closeAt']).map(row=>({...row,submittedElsewhere:otherMonthSubmittedIds.has(String(row.id))})),activityExamIds=new Set([...monthlyAttempts,...monthlyGrades].map(row=>String(row.examId)).filter(Boolean));
  ...
  const payments=source.payments.filter(row=>paymentMonth(row)===monthKey);
  return {
    monthKey,student,sessions:inMonth(source.sessions,['date']),transfers:source.transfers,groupsBySchedule:source.groupsBySchedule,sessionsComplete:source.sessionsComplete===true||student.sessionCalendarCompleteMonths?.includes(monthKey)===true,
...
function reportAvailableMonths(source,currentMonthKey) {
  ...
  const groups=[[source.attendance,['date']],[source.grades,['date','submittedAt','reviewedAt']],[source.examAttempts,['submittedAt','date','startedAt']],[source.exams,['openAt','createdAt','closeAt']],[source.homeworks,['submittedAt']],[source.assignments,['publishAt','createdAt','dueDate']],[source.recitations,['date']],[source.progress,['lastOpenedAt','completedAt']],[source.payments,[]]];
  groups.forEach(([rows,fields])=>rows.forEach(row=>{
    if(rows===source.grades&&row.examId&&source.examAttempts.some(attempt=>String(attempt.examId||'')===String(row.examId)))return;
    if(rows===source.payments&&row.academicYear&&row.month){
      try{keys.add(leaderboardPeriod(String(row.academicYear),String(row.month)).monthKey);}catch(_){}
      return;
    }
    if(rows===source.grades||rows===source.examAttempts||rows===source.homeworks||rows===source.assignments||rows===source.exams){
      const preferred=rows===source.grades?['date','submittedAt','createdAt','reviewedAt']:rows===source.examAttempts?['submittedAt','date','startedAt','createdAt']:rows===source.assignments?['publishAt','createdAt','dueDate']:rows===source.exams?['openAt','createdAt','closeAt']:['submittedAt','date','createdAt','reviewedAt','gradedAt','updatedAt'];
      const key=reportMonthForRow(row,preferred);if(/^\d{4}-\d{2}$/.test(key))keys.add(key);
      return;
    }
...
async function buildStudentMonthlyReport(student,monthKey,options={}) {
  ...
  if(cached?.report?.schemaVersion===11&&cached.report.policyVersion==='monthly-v12-scheduled-session-attendance'&&!cached.invalidatedAt&&cached.sourceRevision===sourceRevision&&cached.contentRevision===contentRevision&&options.force!==true)return cached.report;
...
exports.getStudentMonthlyReportAdmin = onCall({ ...CALLABLE_OPTIONS, timeoutSeconds:60, memory:'512MiB' }, async request => {
  await requireStaff(request,['admin']);
...
exports.getParentMonthlyReport = onCall({ ...CALLABLE_OPTIONS, timeoutSeconds:60, memory:'512MiB' }, async request => {
  const studentCode=normalizeCode(request.data?.studentCode),monthKey=text(request.data?.monthKey,7);
  await requirePortalSession(request,studentCode,['parent']);
  const found=await getStudentPortalByCode(studentCode);
  const [report,ranking]=await Promise.all([buildStudentMonthlyReport(found.data,monthKey),studentReportRanking(found.data,monthKey).catch(()=>null)]);
...
exports.getAdminOperationsDashboard = onCall({ ...CALLABLE_OPTIONS, timeoutSeconds: 60, memory: '512MiB' }, async request => {
  ...
  const paymentPeriod=periodFromMonthKey(date.slice(0,7));
  const [studentsSnap,monthAttendanceSnap,homeworkSnap,examSnap,bookingsSnap,transferSnap,monthSessionsSnap,examAbsenceSnap,approvedTransfersSnap,paymentSnap,groupsSnap]=await Promise.all([
    fetchAllCollectionDocuments('students',query=>query.where('active','==',true)),
    fetchAllCollectionDocuments('attendance',query=>query.where('date','>=',monthStart).where('date','<=',date)),
    ...
    fetchAllCollectionDocuments('class_sessions',query=>query.where('date','>=',monthStart).where('date','<=',date)),
    db.collection('exam_absences').orderBy('createdAt','desc').limit(300).get().catch(()=>null),
    fetchAllCollectionDocuments('student_transfer_requests',query=>query.where('status','==','approved')),
    fetchAllCollectionDocuments('monthly_payments',query=>query.where('academicYear','==',paymentPeriod.academicYear).where('month','==',paymentPeriod.monthName)),
    fetchAllCollectionDocuments('groups')
  ]);
  const students=studentsSnap.docs.map(doc=>({id:doc.id,...doc.data()}));
  const monthRows=monthAttendanceSnap.docs.map(doc=>({id:doc.id,...doc.data()})),monthSessions=monthSessionsSnap.docs.map(doc=>({id:doc.id,...doc.data()}));
  const transfers=approvedTransfersSnap.docs.map(doc=>({id:doc.id,...doc.data()}));
  const groupsBySchedule=new Map(groupsSnap.docs.map(doc=>[String(doc.id),doc.data()]));
  const paymentByStudent=new Map();
  for(const doc of paymentSnap.docs){const row=doc.data(),code=normalizeCode(row.studentCode);if(!code)continue;const total=paymentByStudent.get(code)||{expectedAmount:0,paidAmount:0};total.expectedAmount+=money(row.expectedAmount);total.paidAmount+=money(row.paidAmount);paymentByStudent.set(code,total);}
  const isPaid=row=>{const total=paymentByStudent.get(normalizeCode(row.studentCode||row.code||row.id));return !!total&&paymentStatus(total.expectedAmount,total.paidAmount)==='paid';};
  ...
  const attendanceMetrics=students.map(student=>{
    const studentCode=normalizeCode(student.studentCode||student.code||student.id),history=transfers.filter(row=>normalizeCode(row.studentCode)===studentCode),records=monthRows.filter(row=>normalizeCode(row.studentCode||row.studentId||row.code)===studentCode);
    const scheduleIds=new Set([student.scheduleId||student.groupId,...history.flatMap(row=>[row.currentScheduleId,row.targetScheduleId,row.previousScheduleId,row.oldScheduleId,row.newScheduleId])].filter(Boolean).map(String));
    const sessions=actualSessionsForStudent(student,monthSessions.filter(row=>scheduleIds.has(String(row.scheduleId||row.groupId||''))),records,history,groupsBySchedule);
    const result=calculateMonthlyReport({student,monthKey:date.slice(0,7),now:new Date(`${date}T23:59:59+03:00`),sessions,sessionsComplete:true,attendance:records,transfers:history,groupsBySchedule}).attendance;
    return {student,studentCode,result};
  });
  const atRisk=attendanceMetrics.map(({student,studentCode,result})=>({studentCode,studentName:text(student.studentName||student.name,100),grade:text(canonicalAcademicLabel(student.grade),80),group:text(student.group,100),absenceCount:result.absent})).filter(row=>row.absenceCount>=2).sort((a,b)=>b.absenceCount-a.absenceCount).slice(0,50);
  const unrecordedToday=attendanceMetrics.reduce((sum,{result})=>sum+result.rows.filter(row=>row.date===date&&row.method==='session_without_attendance').length,0);
  const attendance=attendanceMetrics.flatMap(({result})=>result.rows.filter(row=>row.date===date&&row.method!=='session_without_attendance'));
  return {date,students:students.map(row=>({studentCode:text(row.studentCode||row.code||row.id,40),studentName:text(row.studentName||row.name,100),grade:text(canonicalAcademicLabel(row.grade),80),group:text(row.group,100),scheduleId:text(row.scheduleId||row.groupId,100),parentPhone:digits(row.parentPhone),paid:isPaid(row)})),attendance,sessions:monthSessions.filter(row=>row.date===date),corrections:{homework:pendingHomework,exams:pendingExams,total:pendingHomework.length+pendingExams.length},alerts:{pendingBookings:bookingsSnap?.size||0,pendingTransfers:transferSnap?.size||0,unpaidStudents:students.filter(row=>!isPaid(row)).length,unrecordedToday,atRisk,examAbsences}};
});
...
exports.getClassSessionWorkspace = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const [studentsSnap,attendanceSnap,recitationsSnap,transfers]=await Promise.all([db.collection('students').where('active','==',true).limit(251).get(),db.collection('attendance').where('date','==',date).limit(501).get(),db.collection('recitations').where('date','==',date).where('scheduleId','==',scheduleId).limit(1000).get().catch(()=>null),attendanceTransferHistoryBatch()]);
  if(studentsSnap.size>250||attendanceSnap.size>500)throw new HttpsError('resource-exhausted','سجلات الحصة تجاوزت حد العرض الآمن.');
  const attendance=new Map(),recitations=new Map((recitationsSnap?.docs||[]).map(doc=>[normalizeCode(doc.data().studentCode),{id:doc.id,...doc.data()}]));
  for(const doc of attendanceSnap.docs){const row=doc.data(),sessionSpecific=String(row.classSessionId||row.sessionId||'')===sessionId;
    if(!sessionSpecific&&(row.classSessionId||row.sessionId||row.scheduleId&&row.scheduleId!==scheduleId||row.group&&row.group!==session.group))continue;
    const code=normalizeCode(row.studentCode||row.studentId||row.code),previous=attendance.get(code);
    if(!previous||sessionSpecific&&!previous.classSessionId&&!previous.sessionId)attendance.set(code,{id:doc.id,...row});
  }
  return {session,students:studentsSnap.docs.map(doc=>({id:doc.id,...doc.data()})).filter(row=>row.active!==false&&membershipAt(row,transfers.get(normalizeCode(row.studentCode||row.code||row.id))||[],date)?.scheduleId===scheduleId).map(row=>{const studentCode=normalizeCode(row.studentCode||row.code||row.id),membership=membershipAt(row,transfers.get(studentCode)||[],date);return {studentCode,studentName:text(row.studentName||row.name,100),grade:text(canonicalAcademicLabel(row.grade),80),group:text(membership.group,100),attendance:attendance.get(studentCode)||null,recitation:recitations.get(studentCode)||null};})};
});
...
exports.reviewExamAttempt = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const result = await db.runTransaction(async tx => {
    ...
    const summary = {
      ...
      attemptNumber: Math.max(1, Number(attempt.attemptNumber || attempt.attemptSequence || 1)),
      score,
    ...
    // Invalidate before returning the review result; the Firestore trigger can
    // arrive after the next report request (including for an earlier month).
    tx.set(db.collection('monthly_report_state').doc(studentCode),{version:FieldValue.increment(1),updatedAt:FieldValue.serverTimestamp()},{merge:true});
...
const leaderboardCacheByPeriod = new Map();
...
function leaderboardRecordDate(row = {}) {
  return cairoDateKey(row.submittedAt || row.date || row.examDate || row.createdAt || row.updatedAt || '');
}
...
const DEFAULT_MOTIVATION_CONFIG={weights:{exams:30,attendance:25,homeworkCompletion:20,homeworkGrade:10,practical:10,teacherBonus:5},penalties:{absence:2,consecutiveAbsence:5,missedExam:5,missingHomework:2,lateHomework:1}};
function boundedMotivationNumber(value,fallback,min=0,max=100){const parsed=Number(value);return Number.isFinite(parsed)?Math.max(min,Math.min(max,parsed)):fallback;}
function motivationConfigFromSettings(settings={}){const raw=settings.motivationConfig||{},weights=raw.weights||{},penalties=raw.penalties||{};return {weights:{exams:boundedMotivationNumber(weights.exams,30),attendance:boundedMotivationNumber(weights.attendance,25),homeworkCompletion:boundedMotivationNumber(weights.homeworkCompletion,20),homeworkGrade:boundedMotivationNumber(weights.homeworkGrade,10),practical:boundedMotivationNumber(weights.practical,10),teacherBonus:boundedMotivationNumber(weights.teacherBonus,5,0,20)},penalties:{absence:boundedMotivationNumber(penalties.absence,2,0,20),consecutiveAbsence:boundedMotivationNumber(penalties.consecutiveAbsence,5,0,30),missedExam:boundedMotivationNumber(penalties.missedExam,5,0,30),missingHomework:boundedMotivationNumber(penalties.missingHomework,2,0,20),lateHomework:boundedMotivationNumber(penalties.lateHomework,1,0,20)}};}
...
function previousLeaderboardPeriod(){
  const current=leaderboardPeriod(),[year,month]=current.monthKey.split('-').map(Number),date=new Date(Date.UTC(year,month-2,1));
  const previousYear=date.getUTCFullYear(),previousMonth=date.getUTCMonth()+1;
  return {monthKey:`${previousYear}-${String(previousMonth).padStart(2,'0')}`,monthName:PAYMENT_MONTH_NAMES[previousMonth-1]||'',academicYear:previousMonth>=7?`${previousYear}/${previousYear+1}`:`${previousYear-1}/${previousYear}`};
}

function previousPeriodFor(period){const [year,month]=period.monthKey.split('-').map(Number),date=new Date(Date.UTC(year,month-2,1)),previousYear=date.getUTCFullYear(),previousMonth=date.getUTCMonth()+1;return {monthKey:`${previousYear}-${String(previousMonth).padStart(2,'0')}`,monthName:PAYMENT_MONTH_NAMES[previousMonth-1]||'',academicYear:previousMonth>=7?`${previousYear}/${previousYear+1}`:`${previousYear-1}/${previousYear}`};}
function periodFromMonthKey(monthKey){const match=String(monthKey||'').match(/^(\d{4})-(\d{2})$/);if(!match)return leaderboardPeriod();const year=Number(match[1]),month=Number(match[2]);return {monthKey,monthName:PAYMENT_MONTH_NAMES[month-1]||'',academicYear:month>=7?`${year}/${year+1}`:`${year-1}/${year}`};}

async function leaderboardRowsForPeriod(academicYear='',monthName='',options={}) {
  ...
  if(period.monthKey!==currentPeriod.monthKey&&options.recalculate!==true&&options.includeAll!==true){
    ...
    if(archived&&!archived.empty&&archived.docs.every(doc=>Number(doc.data()?.sourceVersion)===stateVersion&&doc.data()?.attendancePolicyVersion==='monthly-v12-scheduled-session-attendance'))
  ...
  const [studentsSnap, attendanceSnap, gradesSnap, examAttemptsSnap, homeworkSnap, recitationSnap, assignmentSnap, motivationSnap, examAbsenceSnap,settingsSnap,sessionsSnap,transfersSnap,groupsSnap] = await Promise.all([
    fetchAllCollectionDocuments('students', query => query.where('active', '==', true)),
    ...
    fetchAllCollectionDocuments('exam_absences'),
    db.collection('settings').doc('platform').get().catch(()=>null),
    fetchAllCollectionDocuments('class_sessions',query=>query.where('date','>=',periodStart).where('date','<',periodEnd)),
    fetchAllCollectionDocuments('student_transfer_requests',query=>query.where('status','==','approved')),
    fetchAllCollectionDocuments('groups')
  ]);
  ...
  const attendance=grouped(attendanceSnap),grades=grouped(gradesSnap),examAttempts=grouped(examAttemptsSnap),homeworks=grouped(homeworkSnap),recitations=grouped(recitationSnap),motivation=grouped(motivationSnap),examAbsences=grouped(examAbsenceSnap),config=motivationConfigFromSettings(settingsSnap?.exists?settingsSnap.data():{});
  const sessionRows=sessionsSnap.docs.map(doc=>({id:doc.id,...doc.data()})),transfers=grouped(transfersSnap);
  const groupsBySchedule=new Map(groupsSnap.docs.map(doc=>[String(doc.id),doc.data()]));
  ...
  const calculatedRows=studentsSnap.docs.map(doc=>{
    ...
    const monthlyEvaluation=calculateMonthlyReport({monthKey:period.monthKey,student:st,transfers:studentTransfers,
      sessions:actualSessionsForStudent(st,applicableSessions,att,studentTransfers,groupsBySchedule),sessionsComplete:true,attendance:att,groupsBySchedule,
    ...
    const classDates=new Set(attendanceResult.rows.map(recordDate).filter(Boolean));rec.forEach(row=>{const date=recordDate(row);if(date)classDates.add(date);});
    const sessions=classDates.size,completedDates=items=>new Set(items.map(recordDate).filter(Boolean)).size;
    const gradePct=monthlyEvaluation.results.average,homeworkPct=monthlyEvaluation.homework.completionPercentage,homeworkGradePct=monthlyEvaluation.homework.averageGrade,recitationPct=sessions?Math.min(100,Math.round(completedDates(rec)/sessions*100)):0;
    const manualMotivationPoints=monthlyEvaluation.motivation.manualPoints,motivationPoints=monthlyEvaluation.motivation.totalPoints;
    ...
    const teacherLimit=config.weights.teacherBonus,motivationBonus=Math.max(-teacherLimit,Math.min(teacherLimit,manualMotivationPoints));
    const weightTotal=(gradePct===null?0:config.weights.exams)+(attendancePct===null?0:config.weights.attendance)+(homeworkPct===null?0:config.weights.homeworkCompletion)+(homeworkGradePct===null?0:config.weights.homeworkGrade)+(rec.length?config.weights.practical:0)||1;
...
function enrichLeaderboardRows(rows,previousRows=[]){
  ...
  const groupTotals=new Map();rows.forEach(row=>{const key=row.scheduleId||row.group||row.grade||'all';groupTotals.set(key,(groupTotals.get(key)||0)+1);});
  return rows.map((row,index)=>{
    const groupKey=row.scheduleId||row.group||row.grade||'all',groupRank=(groupCounters.get(groupKey)||0)+1;groupCounters.set(groupKey,groupRank);
    const previous=previousByCode.get(row.studentCode),rank=index+1,rankDelta=previous?previous.rank-rank:null,scoreDelta=previous?Math.round(Number(row.score||0)-Number(previous.score||0)):null;
    const achievements=[...(row.achievements||[])];if(scoreDelta>=10&&!achievements.includes('الأكثر تحسنًا'))achievements.push('الأكثر تحسنًا');if(previous&&Number(previous.score||0)<60&&Number(row.score||0)>=70)achievements.push('عودة قوية');
    const nextRankGap=index>0?Math.max(0,Number(rows[index-1].score||0)-Number(row.score||0)+1):0;
    return {...row,rank,groupRank,groupTotalStudents:groupTotals.get(groupKey)||0,previousRank:previous?.rank||null,rankDelta,scoreDelta,previousScore:previous?Number(previous.score||0):null,nextRankGap,achievements};
  });
}

async function studentReportRanking(student,monthKey){
  const studentCode=normalizeCode(student.studentCode||student.code||student.id),grade=canonicalLeaderboardGrade(student.grade),period=periodFromMonthKey(monthKey),currentRows=await leaderboardRowsForPeriod(period.academicYear,period.monthName,{recalculate:true});
...
async function leaderboardRowsForGradeWithFallback(grade){
  const normalized=canonicalLeaderboardGrade(grade),current=leaderboardPeriod();
  let rows=(await leaderboardRowsForPeriod()).filter(row=>canonicalLeaderboardGrade(row.grade)===normalized);
  if(rows.length){const previous=previousPeriodFor(current),previousRows=(await leaderboardRowsForPeriod(previous.academicYear,previous.monthName)).filter(row=>canonicalLeaderboardGrade(row.grade)===normalized);return {rows:enrichLeaderboardRows(rows,previousRows),period:current,isPreviousPeriod:false};}
  const previous=previousLeaderboardPeriod();
  rows=(await leaderboardRowsForPeriod(previous.academicYear,previous.monthName)).filter(row=>canonicalLeaderboardGrade(row.grade)===normalized);
  const beforePrevious=previousPeriodFor(previous),beforeRows=(await leaderboardRowsForPeriod(beforePrevious.academicYear,beforePrevious.monthName)).filter(row=>canonicalLeaderboardGrade(row.grade)===normalized);
  return {rows:enrichLeaderboardRows(rows,beforeRows),period:previous,isPreviousPeriod:true};
}
...
exports.getPublicLeaderboard = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const result=await leaderboardRowsForGradeWithFallback(requestedGrade);
  return result.rows.slice(0,5).map(({studentCode,scheduleId,group,activeAbsenceDates,...row})=>({...row,periodMonth:result.period.monthName,periodMonthKey:result.period.monthKey,isPreviousPeriod:result.isPreviousPeriod}));
});
...
exports.getStudentLeaderboardPosition = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const leaderboardResult=await leaderboardRowsForGradeWithFallback(grade),gradeRows=leaderboardResult.rows;
  const index = gradeRows.findIndex(row => row.studentCode === studentCode);
  if (index < 0) return { rank: null, groupRank:null,totalStudents: gradeRows.length, score: 0, baseScore: 0, motivationPoints: 0, motivationBonus: 0, grade,periodMonth:leaderboardResult.period.monthName,periodMonthKey:leaderboardResult.period.monthKey,isPreviousPeriod:leaderboardResult.isPreviousPeriod,level:'بيانات غير كافية',achievements:[],penaltyReasons:[],nextAction:'ابدأ بحضور الحصص وتسليم الواجبات المفتوحة' };
  const row = gradeRows[index];
  return { ...row,name:undefined,studentCode:undefined,rank:index+1,totalStudents:gradeRows.length,grade,periodMonth:leaderboardResult.period.monthName,periodMonthKey:leaderboardResult.period.monthKey,isPreviousPeriod:leaderboardResult.isPreviousPeriod };
});
...
exports.migrateStudentCodeSafely = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const rootCollections=['attendance','grades','recitations','homework_submissions','exam_attempts','monthly_payments','payment_transactions','monthly_reports','bookings','booking_status','student_transfer_requests','homework_attempt_grants','student_notes','parent_report_deliveries'];
  const result=await db.runTransaction(async tx=>{
...
async function attendanceTransferHistory(studentCode){
  const snap=await db.collection('student_transfer_requests').where('studentCode','==',normalizeCode(studentCode)).limit(101).get();
  if(snap.size>100)throw new HttpsError('resource-exhausted','سجل نقل الطالب كبير؛ راجعه قبل تسجيل الحضور التاريخي.');
  return snap.docs.map(doc=>doc.data()).filter(row=>row.status==='approved');
}
async function attendanceTransferHistoryBatch(){
  const snap=await db.collection('student_transfer_requests').where('status','==','approved').limit(501).get();
  if(snap.size>500)throw new HttpsError('resource-exhausted','سجل النقل كبير؛ راجعه قبل إنهاء الحضور.');
  const byStudent=new Map();
  snap.docs.forEach(doc=>{const row=doc.data(),code=normalizeCode(row.studentCode);if(!byStudent.has(code))byStudent.set(code,[]);byStudent.get(code).push(row);});
  return byStudent;
}
async function validateAttendanceSchedule(student, date, transfers) {
  const history=transfers||await attendanceTransferHistory(student.studentCode||student.code||student.id);
  const membership=membershipAt(student,history,date);
  if(!membership)throw new HttpsError('failed-precondition','لا يمكن تسجيل الحضور قبل تاريخ انضمام الطالب.');
  let days = [];
  let resolvedScheduleId = text(membership.scheduleId, 100);
  ...
  if (!groupResolved && membership.group) {
    const groupName = text(membership.group, 100);
    const candidates = new Map();
  ...
  // Student scheduleDays is only a last resort for records with no resolvable group.
  if (!days.length && !resolvedScheduleId && !history.length) days = configuredScheduleDays(student.scheduleDays);
  const decision=assertAttendanceDay(days,date);
  return { days:decision.days, weekday:decision.weekday, scheduleId:resolvedScheduleId, group:membership.group };
}

function assertAttendanceDay(days,date){
  const decision=attendanceDayDecision(days,date);
  if(!decision.allowed)throw new HttpsError('failed-precondition',decision.reason==='missing-schedule'?'مواعيد المجموعة غير محددة؛ حدد أيام المجموعة قبل تسجيل الحضور.':'هذا اليوم خارج مواعيد مجموعة الطالب.');
  return decision;
}
...
function attendanceServerPayload(student, date, status, method, staff, scannedAt='') {
  const studentCode = normalizeCode(student.studentCode || student.code || student.id);
  const scannedDate=scannedAt?new Date(scannedAt):new Date(),safeScannedDate=Number.isFinite(scannedDate.getTime())?scannedDate:new Date();
  return {
    ...
    status,
    date,
    time: status === 'present' ? new Intl.DateTimeFormat('en-GB', { timeZone: 'Africa/Cairo', hour: '2-digit', minute: '2-digit', hour12: false }).format(safeScannedDate) : '',
    timeZone: 'Africa/Cairo',
    method,
    scannedBy: staff.email || staff.uid,
    scannedAtDevice: scannedAt?safeScannedDate.toISOString():'',
    recordedByUid: staff.uid,
...
exports.recordAttendance = onCall(CALLABLE_OPTIONS, async request => {
  ...
  payload.scheduleId = attendanceSchedule.scheduleId;
  payload.group = attendanceSchedule.group;
  if(!payload.scheduleId)throw new HttpsError('failed-precondition','اربط الطالب بمجموعة لها حصة فعلية أولًا.');
  ...
  }else{throw new HttpsError('failed-precondition','أنشئ جلسة حصة فعلية لهذا اليوم قبل تسجيل الحضور.');}
...
exports.prepareOfflineAttendance = onCall({...CALLABLE_OPTIONS,timeoutSeconds:60,memory:'512MiB'},async request=>{
  ...
  const [students,transfers]=await Promise.all([db.collection('students').where('active','==',true).limit(251).get(),attendanceTransferHistoryBatch()]);
  if(students.size>250)throw new HttpsError('resource-exhausted','قائمة الطلاب أكبر من حد التجهيز.');
  const roster=students.docs.map(doc=>({id:doc.id,...doc.data()})).filter(row=>row.active!==false&&row.attendanceCode&&membershipAt(row,transfers.get(normalizeCode(row.studentCode||row.code||row.id))||[],date)?.scheduleId===scheduleId).map(row=>({studentCode:normalizeCode(row.studentCode||row.code||row.id),attendanceCode:row.attendanceCode,name:row.studentName||row.name||'',studentName:row.studentName||row.name||'',grade:row.grade||'',group:group.name||group.group||'',scheduleId,scheduleDays:String(group.days||group.scheduleDays||''),active:true}));
...
exports.syncOfflineAttendance = onCall({...CALLABLE_OPTIONS,timeoutSeconds:60,memory:'512MiB'},async request=>{
  ...
  const results=[],preparations=new Map(),groupDecisions=new Map(),transfers=await attendanceTransferHistoryBatch();
  for(const event of events){
    ...
    try{
      ...
      const studentCode=normalizeCode(event.studentCode),date=text(event.date,10),attendanceStatus=event.attendanceStatus==='absent'?'absent':'present',scannedMillis=Date.parse(event.scannedAt);
      ...
      const student={id:studentSnap.id,...studentSnap.data()};
      const membership=membershipAt(student,transfers.get(studentCode)||[],date);
      if(membership?.scheduleId!==preparation.scheduleId||sessionSnap.data().date!==date||String(sessionSnap.data().scheduleId||sessionSnap.data().groupId||'')!==preparation.scheduleId)throw new HttpsError('failed-precondition','الطالب لم يكن في هذه المجموعة بتاريخ الحصة أو الحصة غير مطابقة.');
      if(!groupDecisions.has(preparation.scheduleId)){
        const groupSnap=await db.collection('groups').doc(cleanDocId(preparation.scheduleId)).get();
        groupDecisions.set(preparation.scheduleId,groupSnap.exists?groupSnap.data():null);
      }
      const group=groupDecisions.get(preparation.scheduleId);
      if(!group)throw new HttpsError('failed-precondition','المجموعة غير موجودة.');
      assertAttendanceDay(group.days||group.scheduleDays,date);
      ...
      const payload=attendanceServerPayload(student,date,attendanceStatus,attendanceStatus==='absent'?'offline_roster_finalize':'offline_qr_sync',staff,new Date(scannedMillis).toISOString());
...
exports.bulkMarkAttendance = onCall({ ...CALLABLE_OPTIONS, timeoutSeconds: 60, memory: '512MiB' }, async request => {
  ...
  if(!scheduleId)throw new HttpsError('failed-precondition','حدد معرّف المجموعة لغياب الحصة.');
  const [studentsSnap,attendanceSnap,transfers,groupSnap]=await Promise.all([
    db.collection('students').where('active','==',true).limit(251).get(),
    db.collection('attendance').where('date','==',date).limit(501).get(),
    attendanceTransferHistoryBatch(),db.collection('groups').doc(scheduleId).get()
  ]);
  if(studentsSnap.size>250||attendanceSnap.size>500)throw new HttpsError('resource-exhausted','سجلات اليوم تجاوزت حد الحضور الجماعي الآمن.');
  if(!groupSnap.exists)throw new HttpsError('not-found','المجموعة غير موجودة.');
  assertAttendanceDay(groupSnap.data().days||groupSnap.data().scheduleDays,date);
  const students=studentsSnap.docs.map(doc=>({id:doc.id,...doc.data()})).filter(student=>student.active!==false&&(!grade||sameAcademicValue(student.grade,grade))&&membershipAt(student,transfers.get(normalizeCode(student.studentCode||student.code||student.id))||[],date)?.scheduleId===scheduleId);
  if (!students.length) throw new HttpsError('not-found', 'لا يوجد طلاب مستحقون في هذه المجموعة بتاريخ الحصة.');
  const sessionId=cleanDocId(text(body.classSessionId,120)||`${scheduleId}_${date}`);
  const sessionSnap=await db.collection('class_sessions').doc(sessionId).get();
  if(!sessionSnap.exists||sessionSnap.data().date!==date||String(sessionSnap.data().scheduleId||sessionSnap.data().groupId||'')!==scheduleId||sessionSnap.data().cancelled===true||sessionSnap.data().status==='cancelled')throw new HttpsError('failed-precondition','أنشئ جلسة حصة فعلية وصحيحة قبل إنهاء الحضور.');
  const eligibleCodes=new Set(students.map(student=>normalizeCode(student.studentCode||student.code||student.id)));
  const existingCodes=new Set();
  for(const doc of attendanceSnap.docs){const row=doc.data(),code=normalizeCode(row.studentCode||row.studentId||row.code);
    if(!eligibleCodes.has(code))continue;
    if(row.classSessionId||row.sessionId){if(String(row.classSessionId||row.sessionId)!==sessionId)continue;}
    else if(row.scheduleId&&row.scheduleId!==scheduleId||!row.scheduleId&&row.group&&row.group!==(sessionSnap.data().group||groupSnap.data().name||group))continue;
    if(!['present','absent','late','excused','حاضر','غائب','متأخر','بعذر'].includes(row.status))throw new HttpsError('failed-precondition',`راجع حالة الحضور القديمة للطالب ${code} قبل إنهاء الحصة.`);
...
exports.savePaperExamGradesAdmin = onCall({ ...CALLABLE_OPTIONS, timeoutSeconds:120, memory:'512MiB' },async request=>{
  ...
  const [oldSnap,studentsSnap,attemptsSnap,transfers]=await Promise.all([
    ...
    results.length?attendanceTransferHistoryBatch():Promise.resolve(new Map())
  ]);
  ...
  const month=PAYMENT_MONTH_NAMES[Number(examDate.slice(5,7))-1],entries=[];let revisions=0;
  for(const row of results){
    const code=normalizeCode(row.studentCode),student=students.get(code),attemptId=cleanDocId(`${examId}_${code}_paper`);
    const membership=student&&membershipAt(student,transfers.get(code)||[],examDate);
    if(!membership||!learningTargetMatchesStudent(exam,{...student,...membership,groupId:membership.scheduleId}))throw new HttpsError('permission-denied',`الطالب ${code} غير موجود أو غير مستهدف بالامتحان.`);
    ...
    entries.push({row,code,student,membership,attemptId,periodId:motivationPeriodId(code,academicYear,month)});
  }
  let changed=false;
  await db.runTransaction(async tx=>{
    changed=false;
    const attemptRefs=entries.map(entry=>db.collection('exam_attempts').doc(entry.attemptId));
    const summaryRefs=entries.map(entry=>db.collection('motivation_monthly').doc(entry.periodId));
    const snaps=await tx.getAll(examRef,...attemptRefs,...summaryRefs);
    const currentExam=snaps[0];
    if(currentExam.exists&&currentExam.data().assessmentMode!=='paper')throw new HttpsError('failed-precondition','هذا المعرّف يخص امتحانًا إلكترونيًا.');
    if(currentExam.exists&&((attemptsSnap?.size||0)>0||snaps.slice(1,1+entries.length).some(snap=>snap.exists))&&['examDate','totalScore','grade','group','scheduleId','academicYear','term','required'].some(field=>String(currentExam.data()[field]??'')!==String(exam[field]??'')))throw new HttpsError('failed-precondition','لا يمكن تغيير بيانات الامتحان بعد تسجيل النتائج.');
    changed=!currentExam.exists||Object.keys(exam).some(field=>String(currentExam.data()?.[field]??'')!==String(exam[field]??''));
    if(changed)tx.set(examRef,{...exam,updatedAt:FieldValue.serverTimestamp(),updatedByUid:staff.uid,...(!currentExam.exists?{createdAt:FieldValue.serverTimestamp()}: {})},{merge:true});
    for(let index=0;index<entries.length;index++){
      const {row,code,student,membership,attemptId,periodId}=entries[index],previous=snaps[1+index],summary=snaps[1+entries.length+index];
      const old=previous.exists?previous.data():{},absent=row.status==='absent',score=absent?null:Number(row.score);
      const contribution=absent?0:score,oldContribution=Number(old.paperMotivationContribution||0),delta=contribution-oldContribution;
      const changedAttempt=!previous.exists||old.status!==row.status||old.score!==score||old.maxScore!==totalScore||String(old.note||'')!==text(row.note,500)||String(old.scheduleId||'')!==String(membership.scheduleId||'')||String(old.group||'')!==String(membership.group||'')||old.paperMotivationContribution===undefined;
      if(!changedAttempt)continue;
      changed=true;
      const revision=Number(old.paperMotivationRevision||0)+(delta!==0?1:0);
      const payload={id:attemptId,examId,examTitle:title,studentCode:code,studentName:text(student.studentName||student.name,100),grade:text(student.grade,80),group:text(membership.group,100),scheduleId:text(membership.scheduleId,100),academicYear,term,assessmentMode:'paper',attemptType:'paper',source:'center-paper',required,submittedAt:`${examDate}T12:00:00+03:00`,examDate,score,maxScore:totalScore,status:row.status,needsManualReview:false,approved:!absent,note:text(row.note,500),paperMotivationContribution:contribution,paperMotivationRevision:revision,updatedAt:FieldValue.serverTimestamp(),reviewedAt:absent?null:FieldValue.serverTimestamp(),reviewedByUid:staff.uid};
      if(!previous.exists)payload.createdAt=FieldValue.serverTimestamp();
      tx.set(attemptRefs[index],payload,{merge:true});
      invalidateStudentReportInTransaction(tx,code,academicYear,month,'paper-exam-grades');
      if(delta===0)continue;
      const current=summary.exists?summary.data():{},totalPoints=Number(current.totalPoints||0)+delta,paperExamPoints=Number(current.paperExamPoints||0)+delta;
      const transactionRef=db.collection('motivation_transactions').doc(hash(`paper_exam|${examId}|${code}|${revision}`).slice(0,48));
      tx.create(transactionRef,{studentCode:code,studentName:payload.studentName,grade:payload.grade,group:payload.group,scheduleId:payload.scheduleId,academicYear,month,periodId,points:delta,reason:text(`درجة امتحان ورقي: ${title}`,160),source:'paper_exam',referenceId:examId,paperExamScore:score,paperExamMaxScore:totalScore,totalAfter:totalPoints,recordedByUid:staff.uid,recordedByEmail:staff.email||'',recordedByRole:'system',createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()});
      tx.set(summaryRefs[index],{periodId,studentCode:code,studentName:payload.studentName,grade:payload.grade,group:payload.group,scheduleId:payload.scheduleId,academicYear,month,totalPoints,paperExamPoints,transactionCount:Number(current.transactionCount||0)+1,lastReason:text(`درجة امتحان ورقي: ${title}`,160),lastPoints:delta,createdAt:current.createdAt||FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()},{merge:true});
    }
    if(changed)tx.set(leaderboardStateRef,{version:FieldValue.increment(1),reason:'paper-exam-grades',updatedAt:FieldValue.serverTimestamp()},{merge:true});
  });
  if(changed)await serverActivity(staff,oldSnap.exists?'تعديل درجات امتحان ورقي':'إنشاء امتحان ورقي',{examId,examDate,studentCount:results.length,revisions});
...
function examIsPublished(exam) {
  return exam && exam.active!==false && exam.published!==false && exam.archived!==true && exam.cancelled!==true && !['مسودة','cancelled','archived'].includes(exam.status);
}
...
async function finalizeExamAbsenceRecords(){
  ...
  for(const exam of exams){
    ...
    const expected=students.filter(student=>learningTargetMatchesStudent(exam,student)&&contentAvailableAfterStudentJoined(exam,student,scheduledTimeMillis(exam.closeAt)-1));
...
exports.submitExam = onCall(EXAM_ENTRY_OPTIONS, async request => {
  ...
  const attempt = {
    ...
    submittedAt,
    attemptNumber: Math.max(1, Number(session.attemptSequence || 1)),
    score,
    autoScore,
    maxScore,
    mcqCount,
  ...
  const committedResult = await db.runTransaction(async tx => {
    ...
    const absenceSnap = await tx.get(absenceRef);
    tx.set(attemptRef, attempt);
    if (absenceSnap.exists) tx.delete(absenceRef);
...
exports.deleteStudentSafely = onCall({ region: 'europe-west1', timeoutSeconds: 120, memory: '512MiB' }, async request => {
  ...
  const student = studentSnap.data();
  const relatedCollections = ['attendance','grades','recitations','homework_submissions','exam_attempts','exam_absences','monthly_payments','payment_transactions','student_notes','parent_report_deliveries'];
...
exports.recordLectureProgress = onCall(CALLABLE_OPTIONS, async request => {
  ...
  const savedPercent=await db.runTransaction(async transaction=>{
    ...
    transaction.set(db.collection('monthly_report_state').doc(code),{version:FieldValue.increment(1),updatedAt:FieldValue.serverTimestamp()},{merge:true});
    return nextPercent;
  });
