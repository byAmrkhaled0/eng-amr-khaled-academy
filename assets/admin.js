...
async function cacheOfflineAttendanceRoster(){try{if(window.OfflineAttendance&&(adminData.students||[]).length)await window.OfflineAttendance.cacheRoster(adminData.students);if(navigator.onLine!==false)await ensureQrScannerLibrary();}catch(error){console.warn('offline-attendance-roster-cache',error);}}
