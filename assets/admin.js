...
function examAttemptRowHTML(a){return `<div class="mobile-row exam-review-row-v40" data-exam-attempt-id="${safe(a.id)}"><div><b>${safe(a.studentName||'-')} — ${safe(a.examTitle||'امتحان')}</b><small>${safe(a.studentCode||'-')} · ${safe(a.submittedAt||'-')}</small></div>${examAttemptBadge(a)}<span class="badge ${a.score!==null&&a.score!==undefined?'good':'warn'}">${safe(examAttemptScoreText(a))}</span><div class="mobile-actions"><button class="small-btn primary" type="button" onclick="correctAttempt('${safe(a.id)}')">عرض وتصحيح</button></div></div>`;}
window.correctAttempt=function(id){const a=adminData.examAttempts.find(x=>String(x.id)===String(id)); if(!a)return; document.querySelector('.correction-modal-v40')?.remove(); const answers=Array.isArray(a.answers)?a.answers:[]; const max=Number(a.maxScore||answers.reduce((s,x)=>s+Number(x.mark||1),0)||100); const html=`<div class="correction-modal-v40"><div class="correction-card-v40 card"><div class="profile-top"><div><span class="kicker">تصحيح امتحان</span><h2>${safe(a.studentName||'-')}</h2><p class="section-desc">${safe(a.examTitle||'امتحان')} · الدرجة من ${safe(max)}</p></div><button type="button" class="small-btn danger" onclick="closeCorrectionModal()">إغلاق</button></div><div class="correction-list-v40">${answers.map((ans,i)=>`<div class="correction-question-v40"><h3>${i+1}. ${safe(ans.question||'سؤال')} <small>(${safe(ans.mark||1)} درجة)</small></h3><div class="correction-answer-grid-v40"><div><span>إجابة الطالب</span><p>${safe(ans.answer||'-')}</p></div><div><span>الإجابة الصحيحة / النموذجية</span><p>${safe(ans.correctAnswer||ans.modelAnswer||'يصَححها المدرس')}</p></div></div><label>الدرجة المستحقة<input type="number" data-awarded-index="${i}" min="0" max="${safe(ans.mark||1)}" step="0.25" value="${safe(ans.awardedMark??(ans.correct?ans.mark||1:0))}" oninput="recalculateCorrectionScore()"></label></div>`).join('')||'<p class="section-desc">لا توجد إجابات محفوظة.</p>'}</div><div class="correction-final-v40"><label>الدرجة النهائية من ${safe(max)}<input id="manualFinalScore" type="number" min="0" max="${safe(max)}" step="0.25" value="${safe(a.score??a.autoScore??0)}" readonly></label><button type="button" class="btn primary" onclick="saveAttemptCorrection('${safe(a.id)}')">حفظ التصحيح والنتيجة</button></div></div></div>`; document.body.insertAdjacentHTML('beforeend',html); recalculateCorrectionScore();};
...
window.saveAttemptCorrection=async function(id){
  ...
  const button=document.querySelector('.correction-final-v40 .btn.primary');if(button){button.disabled=true;button.classList.add('is-loading');}
  try{
    if(!window.MFCloud?.reviewExamAttempt)throw new Error('Secure exam correction service unavailable');
    const result=await window.MFCloud.reviewExamAttempt({attemptId:String(id),awarded,reason:'تصحيح الامتحان من لوحة الإدارة'});
    ...
    saveData(adminData);aToast('تم حفظ التصحيح والنتيجة وتحديث بوابة الطالب');closeCorrectionModal();document.querySelectorAll(`[data-exam-attempt-id="${globalThis.CSS?.escape?CSS.escape(String(id)):String(id)}"]`).forEach(row=>{const shell=document.createElement('div');shell.innerHTML=examAttemptRowHTML(attempt);if(shell.firstElementChild)row.replaceWith(shell.firstElementChild);});
