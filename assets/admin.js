...
const adminSections = [
  ...
  ['classLinks','external-link','روابط الحصص'],
...
const adminSectionGroups = [
  ...
  ['التعليم', ['materials','classLinks','theoryLectures','assignments','exams','curriculum']],
...
function renderClassLinks(){fresh();const rows=(adminData.materials||[]).filter(item=>item.resourceType==='class-link'||item.materialType==='class-link'||item.linkUrl);content(`<div class="section-head"><div><span class="kicker"><span data-icon="external-link"></span> مكتبة الحصص</span><h2 class="section-title">روابط تسجيلات الحصص</h2><p class="section-desc">أضف رابط Google Drive وحدد المسار والمجموعة ليظهر للطلاب المناسبين فقط.</p></div></div><div class="class-links-admin-layout"><form id="classLinkForm" class="card grid"><h3>إضافة رابط حصة</h3><label class="field"><span>عنوان الحصة</span><input name="title" required maxlength="200" placeholder="مثال: الحصة الثالثة — المتغيرات"></label><label class="field"><span>رابط Google Drive</span><input name="linkUrl" type="url" inputmode="url" required placeholder="https://drive.google.com/..."></label><label class="field"><span>المسار</span><select name="grade"><option>كل المسارات</option>${GRADES.map(g=>`<option>${safe(g)}</option>`).join('')}</select></label><label class="field"><span>المجموعة</span><select name="group">${adminTargetGroupOptions(true)}</select></label><label class="field"><span>رقم أو اسم الوحدة</span><input name="unit" maxlength="120" placeholder="الوحدة الأولى"></label><label class="field"><span>تاريخ الحصة</span><input name="lecture" maxlength="120" placeholder="5 سبتمبر 2026"></label><label class="field full-span"><span>وصف مختصر</span><textarea name="desc" rows="3" maxlength="1200" placeholder="النقاط التي تم شرحها في الحصة"></textarea></label><button class="btn primary full-span" type="submit"><span data-icon="external-link"></span> حفظ ونشر الرابط</button></form><section class="card class-links-admin-list"><div class="student-panel-title"><div><small>الروابط المنشورة</small><h3>${rows.length} رابط حصة</h3></div></div>${rows.slice().sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))).map(item=>`<article class="class-link-admin-row"><div><b>${safe(item.title)}</b><small>${safe(item.grade||'كل المسارات')} · ${safe(item.group||'كل المجموعات')}</small><a href="${safe(item.linkUrl||item.fileUrl)}" target="_blank" rel="noopener noreferrer">فتح الرابط للتأكد</a></div><button class="small-btn danger" type="button" onclick="deleteItem('materials','${safe(item.id)}')">حذف</button></article>`).join('')||'<div class="portal-empty"><h3>لا توجد روابط بعد</h3><p>أضف أول رابط من النموذج.</p></div>'}</section></div>`);const form=document.getElementById('classLinkForm');bindAdminAcademicTarget('classLinkForm');form.onsubmit=async event=>{event.preventDefault();const values=Object.fromEntries(new FormData(form).entries()),button=form.querySelector('[type="submit"]'),linkUrl=normalizeDriveUrl(values.linkUrl);if(!linkUrl)return aToast('اكتب رابط Google Drive صحيحًا يبدأ بـ https');const scheduleId=selectedScheduleId(form);const item={...values,linkUrl,fileUrl:linkUrl,fileType:'link',resourceType:'class-link',materialType:'class-link',scheduleId,groupId:scheduleId,id:`class-link-${Date.now()}`,active:true,published:true,status:'منشور',createdAt:new Date().toISOString()};button.disabled=true;button.classList.add('is-loading');try{if(!window.MFCloud?.saveContent)throw new Error('Content service unavailable');const saved=await window.MFCloud.saveContent('materials',item);adminData.materials.push({...item,...saved});saveData(adminData);aToast('تم نشر رابط الحصة للطلاب المحددين');renderClassLinks();}catch(error){aToast(adminActionErrorMessage(error,'تعذر حفظ رابط الحصة.'));}finally{button.disabled=false;button.classList.remove('is-loading');}};hydrateIcons();}
...
function renderSection(){({overview:renderOverview,operations:()=>window.renderOperations?.(),students:renderStudents,motivation:renderMotivationAdmin,bookings:renderBookings,schedules:renderSchedules,attendance:renderAttendance,warnings:renderWarnings,studentRequests:renderStudentRequests,assignments:renderAssignments,payments:renderPayments,exams:renderExams,materials:renderMaterials,classLinks:renderClassLinks,theoryLectures:()=>window.renderTheoryLectures?.(),curriculum:()=>window.renderCurriculumAdmin?.(),reviews:renderReviewsAdmin,backup:renderBackup,settings:renderSettings}[currentSection]||renderOverview)();if(currentSection==='students')bindAdminGradeGroupPicker();}
