...
const adminSections = [
  ...
  ['operations','clipboard','مركز العمل'],
...
async function loadAccurateMonthlyReport(st){if(!window.MFCloud?.getStudentMonthlyReportAdmin)throw new Error('Monthly report service unavailable');return window.MFCloud.getStudentMonthlyReportAdmin({studentCode:stCode(st),monthKey:adminReportMonthKey(),force:true,includeRanking:true});}
...
function renderAdmin(){
  ...
  root.innerHTML=`
    ...
    <nav class="admin-mobile-bottom" aria-label="التنقل السريع في لوحة التحكم">
      ${[['operations','clipboard','مركز العمل'],['attendance','qr','الحضور'],['students','users','الطلاب'],['motivation','star','التحفيز']].map(([id,ic,name])=>`<button type="button" data-admin-nav="${id}" class="${id===currentSection?'active':''}"><span data-icon="${ic}"></span><span>${name}</span></button>`).join('')}
...
function renderOverview(){
  ...
  content(`<section class="admin-overview-hero"><div><span class="admin-overview-eyebrow"><i></i> نظرة عامة مباشرة</span><h2>مساء الخير، ${safe(currentStaff?.name||'م. عمرو خالد')}</h2><p>تابع المهام المهمة وابدأ إجراءات اليوم من مكان واحد.</p></div><div class="admin-overview-hero-meta"><small>${safe(today)}</small><strong>${urgent}</strong><span>مهمة تحتاج متابعة</span></div></section><div class="admin-metric-grid admin-action-kpis"><button type="button" class="admin-metric-card metric-bookings" onclick="goAdminSection('bookings')"><span class="admin-metric-icon" data-icon="user-check"></span><span class="admin-metric-copy"><small>طلبات التسجيل</small><b>${s.bookings}</b><em>فتح الطلبات</em></span><span class="admin-metric-arrow">←</span></button><button type="button" class="admin-metric-card metric-exams" onclick="goAdminSection('exams')"><span class="admin-metric-icon" data-icon="clipboard"></span><span class="admin-metric-copy"><small>بانتظار التصحيح</small><b>${pendingExam}</b><em>فتح الامتحانات</em></span><span class="admin-metric-arrow">←</span></button><button type="button" class="admin-metric-card metric-transfers" onclick="goAdminSection('studentRequests')"><span class="admin-metric-icon" data-icon="users"></span><span class="admin-metric-copy"><small>طلبات النقل</small><b>${pendingTransfers}</b><em>مراجعة الطلبات</em></span><span class="admin-metric-arrow">←</span></button><button type="button" class="admin-metric-card metric-payments" onclick="goAdminSection('payments')"><span class="admin-metric-icon" data-icon="database"></span><span class="admin-metric-copy"><small>لم يسدد هذا الشهر</small><b>${s.unpaid}</b><em>فتح المدفوعات</em></span><span class="admin-metric-arrow">←</span></button></div><div class="admin-overview-layout"><section class="card admin-daily-actions admin-focus-panel"><div class="admin-panel-heading"><div><span class="admin-panel-label">اختصارات العمل</span><h3>ابدأ إجراءً جديدًا</h3><p class="section-desc">أكثر الأدوات استخدامًا أثناء اليوم الدراسي.</p></div><span class="admin-panel-heading-icon" data-icon="sparkles"></span></div><div class="admin-task-grid-v37"><button type="button" class="admin-task-button primary-task" onclick="goAdminSection('attendance')"><span data-icon="qr"></span><b>تسجيل الحضور</b><small>مسح QR أو إدخال الكود</small></button><button type="button" class="admin-task-button" onclick="goAdminSection('motivation')"><span data-icon="star"></span><b>تحفيز الطلاب</b><small>النقاط ولوحة الترتيب</small></button><button type="button" class="admin-task-button" onclick="goAdminSection('materials')"><span data-icon="book-open"></span><b>إضافة محاضرة</b><small>رفع ملف أو رابط جديد</small></button><button type="button" class="admin-task-button" onclick="goAdminSection('assignments')"><span data-icon="file-text"></span><b>إنشاء واجب</b><small>الأسئلة وموعد التسليم</small></button><button type="button" class="admin-task-button" onclick="goAdminSection('exams')"><span data-icon="clipboard"></span><b>إنشاء امتحان</b><small>إعداد ونشر الاختبار</small></button></div></section><aside class="card admin-pulse-panel"><div class="admin-panel-heading"><div><span class="admin-panel-label">حالة الأكاديمية</span><h3>ملخص اليوم</h3></div><span class="admin-panel-heading-icon" data-icon="bar-chart"></span></div><div class="admin-pulse-list"><button type="button" onclick="goAdminSection('students')"><span class="admin-pulse-icon" data-icon="users"></span><span><small>إجمالي الطلاب</small><b>${s.students}</b></span></button><button type="button" onclick="goAdminSection('attendance')"><span class="admin-pulse-icon" data-icon="user-check"></span><span><small>حضور اليوم</small><b>${s.att}</b></span></button><button type="button" onclick="goAdminSection('schedules')"><span class="admin-pulse-icon" data-icon="calendar"></span><span><small>المجموعات النشطة</small><b>${activeGroups}</b></span></button></div><div class="admin-system-note"><i></i><span><b>النظام متصل</b><small>التحديثات الناجحة تظهر للطلاب مباشرة</small></span></div></aside></div>`);
}
...
window.printStudentReport=async function(code){
  ...
  let enriched=normalizeStudent(s);try{const history=await window.MFCloud?.getStudentPaymentHistory?.(code);if(history)enriched={...enriched,monthlyPayments:history.summaries||[],paymentTransactions:history.transactions||[]};}catch(error){aToast('تعذر تحميل سجل الدفع؛ سيتم فتح باقي ملف الطالب');}
...
function warningHistory(student){
  const perSession=new Map();
  (student?.attendance||[]).forEach(record=>{
    let status=String(record?.status||'').trim();
    if(['حاضر','متأخر','late'].includes(status))status='present';if(status==='غائب')status='absent';
    if(!['present','absent'].includes(status))return;
    const date=String(record.date||record.createdAt||'').slice(0,10);if(!date)return;
    const key=String(record.sessionId||record.sessionKey||record.id||`${date}:${record.time||''}:${record.scheduleId||record.group||''}`);
    perSession.set(key,{...record,date,status});
  });
  return [...perSession.values()].sort((a,b)=>`${a.date} ${a.time||''}`.localeCompare(`${b.date} ${b.time||''}`));
}
function absenceStreak(student){const rows=warningHistory(student),dates=[];for(let i=rows.length-1;i>=0;i-=1){if(rows[i].status!=='absent')break;dates.push(rows[i].date);}return dates.length>=2?{count:dates.length,dates,latestDate:dates[0]}:null;}
function warningRows(){return (adminData.students||[]).map(normalizeStudent).filter(student=>student.active!==false).map(student=>({student,warning:absenceStreak(student)})).filter(item=>item.warning).sort((a,b)=>b.warning.count-a.warning.count);}
...
window.sendAbsenceWarningWhatsApp=function(code){const student=(adminData.students||[]).map(normalizeStudent).find(item=>String(item.studentCode)===String(code)),warning=absenceStreak(student),phone=adminWhatsAppPhone(student?.parentPhone);if(!student||!warning)return aToast('لا يوجد تحذير غياب صالح');if(!phone)return aToast('رقم ولي الأمر غير مسجل');window.open(whatsappLink(phone,absenceWhatsAppText(student,warning)),'_blank','noopener');};
...
function examAttemptScoreText(a){const max=Number(a.maxScore||100);return (a.score!==null&&a.score!==undefined&&a.score!=='')?`${a.score} من ${max}`:(a.autoScore!==null&&a.autoScore!==undefined?`تلقائي ${a.autoScore} من ${max} · ينتظر النهائي`:'بانتظار التصحيح');}
function examAttemptCorrectionLabel(a){if(a.needsManualReview||a.status==='pending_manual')return 'تصحيح الآن';if(a.status==='corrected')return 'مراجعة وتعديل التصحيح';return 'تعديل التصحيح التلقائي';}
function examAttemptCorrectionNote(a){if(a.needsManualReview||a.status==='pending_manual')return 'هذه المحاولة تنتظر التصحيح. راجع كل إجابة ثم احفظ الدرجة النهائية.';if(a.status==='corrected')return 'سبق حفظ تصحيح هذه المحاولة، ويمكنك تعديل الدرجات مرة أخرى حتى بعد انتهاء الامتحان.';return 'هذه المحاولة صُححت تلقائيًا، ويمكنك تعديل درجة أي سؤال حتى بعد انتهاء الامتحان.';}
