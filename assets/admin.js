...
function adminTargetGroupOptions(includeAll=false){
  ...
  (adminData.groups||[]).filter(group=>group.active!==false).forEach(group=>{const name=group.name||group.group;if(name)groups.set(name,group.grade||'');});
  (adminData.students||[]).forEach(student=>{if(student.group&&!groups.has(student.group))groups.set(student.group,student.grade||student.track||'');});
  const first=includeAll?'<option>كل المجموعات</option>':'<option value="">اختر المجموعة</option>';
  return first+[...groups].map(([name,grade])=>`<option data-grade="${safe(grade)}">${safe(name)}</option>`).join('');
}
...
function bindAdminGradeGroupPicker(){
  ...
  const refresh=()=>{
    ...
    group.innerHTML=rows.length?'<option value="">اختر موعد المسار</option>'+rows.map(item=>`<option value="${safe(item.name||'')}">${safe(scheduleOptionLabel(item))}</option>`).join(''):'<option value="">لا توجد مواعيد لهذا المسار</option>';
    group.required=true;group.disabled=!rows.length;
  };
  grade.addEventListener('change',refresh);refresh();
}
function renderSection(){({overview:renderOverview,students:renderStudents,bookings:renderBookings,schedules:renderSchedules,attendance:renderAttendance,warnings:renderWarnings,studentRequests:renderStudentRequests,assignments:renderAssignments,payments:renderPayments,exams:renderExams,materials:renderMaterials,reviews:renderReviewsAdmin}[currentSection]||renderOverview)();if(currentSection==='students')bindAdminGradeGroupPicker();}
