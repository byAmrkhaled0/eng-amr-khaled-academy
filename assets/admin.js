...
window.sendParentMonthlyReport=async function(code,button){code=String(code||'');if(parentReportDeliveryPending.has(code))return aToast('جارٍ تجهيز تقرير هذا الطالب بالفعل');const s=adminData.students.find(x=>stCode(x)===code);if(!s)return aToast('لم يتم العثور على الطالب');const phone=adminWhatsAppPhone(s.parentPhone);if(!phone)return aToast('رقم ولي الأمر غير موجود؛ افتح تعديل الطالب وسجّل رقم ولي الأمر أولًا');parentReportDeliveryPending.add(code);if(button){button.disabled=true;button.classList.add('is-loading');}aToast('جاري تجهيز أحدث صورة لتقرير ولي الأمر…');try{const report=await loadAccurateMonthlyReport(s);if(report?.student?.studentCode!==code)throw new Error('بيانات التقرير لا تخص الطالب المحدد');if(typeof window.deliverParentMonthlyReport!=='function')throw new Error('خدمة صورة التقرير غير متاحة');const shared=await window.deliverParentMonthlyReport(report,phone,aToast);if(shared)window.MFCloud?.recordParentReportDeliveryAdmin?.({studentCode:code,monthKey:report.monthKey,deliveryMode:window.parentReportDeliveryMode||'image-downloaded'}).catch(error=>console.warn('parent-report-delivery-log',error?.code||error?.message||error));}catch(error){aToast(adminActionErrorMessage(error,'تعذر تجهيز صورة التقرير، ولم يتم فتح رسالة ناقصة.'));}finally{parentReportDeliveryPending.delete(code);if(button){button.disabled=false;button.classList.remove('is-loading');}}};
