...
const adminSectionGroups = [
  ['المتابعة اليومية', ['overview','attendance','students','motivation','schedules']],
...
function renderAdmin(){
  ...
  root.innerHTML=`
    ...
    <nav class="admin-mobile-bottom" aria-label="التنقل السريع في لوحة التحكم">
      ${[['overview','bar-chart','الرئيسية'],['attendance','qr','الحضور'],['students','users','الطلاب'],['motivation','star','التحفيز']].map(([id,ic,name])=>`<button type="button" data-admin-nav="${id}" class="${id===currentSection?'active':''}"><span data-icon="${ic}"></span><span>${name}</span></button>`).join('')}
...
function warningHistory(student){
  ...
  (student?.attendance||[]).forEach((record,index)=>{
    ...
    if(status==='حاضر')status='present';if(status==='غائب')status='absent';
    if(!['present','absent'].includes(status))return;
    const date=String(record.date||record.createdAt||'').slice(0,10);if(!date)return;
    const key=String(record.sessionId||record.sessionKey||record.id||`${date}:${record.scheduleId||record.group||''}:${index}`);
