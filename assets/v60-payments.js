(function(){
  ...
  function applyPaymentResult(row,result,paymentDate){
    ...
    row.summary={...(row.summary||{}),studentCode:row.student.studentCode,course:row.summary?.course||row.student.grade,month:row.month,academicYear:row.academicYear,expectedAmount:row.expected,paidAmount:row.paid,remainingAmount:row.remaining,status:row.status,lastPaymentDate:Object.hasOwn(result,'lastPaymentDate')?result.lastPaymentDate:paymentDate||row.summary?.lastPaymentDate||''};
  ...
  window.markStudentPaid=async function(code,month=currentMonth(),academicYear=schoolYear(),course=''){
    const row=state.rows.find(r=>r.student.studentCode===code&&r.month===month&&r.academicYear===academicYear&&(!course||adminSameAcademic(r.summary?.course||r.student.grade,course)));
  ...
  window.openMonthlyPaymentForm=function(code,month=currentMonth(),academicYear=schoolYear(),transactionId='',course=''){
    ...
    const summary=state.summaries.find(row=>row.studentCode===String(code)&&row.month===month&&row.academicYear===academicYear&&adminSameAcademic(row.course,course||student.grade)),expected=number(summary?.expectedAmount||coursePrice(student)),remaining=Math.max(0,expected-number(summary?.paidAmount));
  ...
  window.saveCoursePrices=async function(){const button=document.getElementById('saveCoursePricesButton');if(button?.disabled)return;const previous={...(adminData.settings?.coursePrices||{})},next={};document.querySelectorAll('[data-course-price]').forEach(input=>{next[input.dataset.coursePrice]=number(input.value);});adminData.settings={...(adminData.settings||{}),coursePrices:next};try{button.disabled=true;button.classList.add('is-loading');await window.MFCloud.saveSettings(adminData.settings);saveData(adminData);applySavedCoursePrices();const editor=document.querySelector('.course-price-editor'),activeGrades=[...new Set((adminData.students||[]).filter(row=>row.active!==false).map(row=>row.grade).filter(Boolean))],missing=activeGrades.filter(grade=>!number(Object.entries(next).find(([key])=>adminSameAcademic(key,grade))?.[1]));if(editor&&!missing.length)editor.open=false;const stateBox=document.getElementById('coursePriceSavedState');if(stateBox)stateBox.textContent=missing.length?`تم الحفظ؛ ما زال ${missing.length} مسار نشط بلا سعر.`:'تم حفظ الأسعار. أي ضغطة «تم الدفع» ستستخدم السعر المحفوظ تلقائيًا.';aToast('تم حفظ الأسعار وتحديث كروت الدفع');scheduleDashboardRefresh();}catch(error){adminData.settings={...(adminData.settings||{}),coursePrices:previous};saveData(adminData);applySavedCoursePrices();aToast(adminActionErrorMessage(error,'تعذر حفظ الأسعار.'));}finally{button.disabled=false;button.classList.remove('is-loading');}};
