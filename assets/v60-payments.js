(function(){
  ...
  window.saveCoursePrices=async function(){const button=document.getElementById('saveCoursePricesButton');if(button?.disabled)return;const previous={...(adminData.settings?.coursePrices||{})},next={};let saved=false;document.querySelectorAll('[data-course-price]').forEach(input=>{next[input.dataset.coursePrice]=number(input.value);});adminData.settings={...(adminData.settings||{}),coursePrices:next};try{button.disabled=true;button.classList.add('is-loading');await window.MFCloud.saveSettings(adminData.settings);saved=true;saveData(adminData);applySavedCoursePrices();await loadDashboard({force:true,background:true});if(state.error)throw new Error('تم حفظ السعر، لكن تعذر تأكيد تحديث لوحة الدفع. اضغط تحديث قبل تسجيل الدفعة.');aToast('تم حفظ الأسعار وتأكيدها للطلاب الحاليين والجدد');}catch(error){if(!saved){adminData.settings={...(adminData.settings||{}),coursePrices:previous};saveData(adminData);applySavedCoursePrices();}aToast(adminActionErrorMessage(error,saved?'تم حفظ السعر، لكن تعذر تحديث لوحة الدفع الآن. اضغط تحديث ثم سجّل الدفعة.':'تعذر حفظ الأسعار.'));}finally{button.disabled=false;button.classList.remove('is-loading');}};
