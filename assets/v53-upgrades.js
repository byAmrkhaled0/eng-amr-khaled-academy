(function(){
  ...
  if(typeof publicDataOnly==='function'){
    publicDataOnly=function(data){
      ...
      return {...defaultData(),materials:d.materials,questions:d.questions,reviews:d.reviews.filter(r=>r.approved===true),groups:d.groups,assignments:d.assignments,settings:{siteUrl:settings.siteUrl||DEFAULT_SITE_URL,teacherPhone:settings.teacherPhone||TEACHER_WHATSAPP,teacherName:settings.teacherName||'م. عمرو خالد',homeNotice:settings.homeNotice||'',academicYear:settings.academicYear||calculatedAcademicYear(),term:settings.term||'الترم الأول'}};
    };
  }
  ...
  function installHelpHTML(){
    ...
    return `<div class="pwa-install-modal" id="pwaInstallModal" role="dialog" aria-modal="true" aria-labelledby="pwaInstallTitle">
      <div class="pwa-install-card card">
        ...
        <span class="pwa-app-icon"><img src="assets/technominds-logo.png" alt="شعار Techno Minds"></span>
        <h2 id="pwaInstallTitle">${isTeacherApp()?'ثبّت لوحة المدرس على الموبايل':'ثبّت Techno Minds على الموبايل'}</h2>
  ...
  function addPasswordReset(){
    ...
    button.onclick=async()=>{
      ...
      if(!email)return typeof aToast==='function'&&aToast('اكتب البريد الإلكتروني أولًا');
      try{await window.MFCloud?.sendPasswordReset?.(email);aToast('تم إرسال رابط إعادة تعيين كلمة المرور إلى البريد');}
      catch(error){aToast('تعذر إرسال الرابط الآن. تأكد من البريد والإنترنت ثم حاول مرة أخرى.');}
    };
  ...
  function initAdminUpgrades(){
    ...
    window.refreshStudentsTable=function(){
      ...
      if(grade!=='all')rows=rows.filter(s=>typeof adminSameAcademic==='function'?adminSameAcademic(s.grade,grade):s.grade===grade);
    ...
    renderStudents=function(){
      ...
      if(toolbar&&!document.getElementById('studentImportTools')){
        toolbar.insertAdjacentHTML('beforebegin',`<div class="card student-import-tools" id="studentImportTools"><div><h3>استيراد وتصدير الطلاب</h3><p>يدعم CSV وExcel. الأعمدة المقبولة: الاسم، هاتف الطالب، هاتف ولي الأمر، المسار، المجموعة، الشهر، العام الدراسي، الترم، الملاحظات.</p></div><div class="mobile-actions"><label class="btn ghost file-button">استيراد CSV / Excel<input type="file" accept=".csv,.xlsx,.xls" onchange="importStudentsFile(this)" hidden></label><button class="btn ghost" type="button" onclick="exportStudentsCSV()">تصدير الطلاب</button></div><div class="import-progress" id="studentImportProgress" hidden></div></div>`);
      }
      populateAcademicSelects();applyAdminRoleGuards();
    };

    window.exportStudentsCSV=function(){
      const rows=adminData.students.map(normalizeStudent).map(s=>[s.studentCode,s.name,s.studentPhone,s.parentPhone,s.grade,s.group,s.month,s.academicYear,s.term,s.paid?'مشترك':'غير مشترك',s.notes]);
      downloadCSV('students-v62.9.csv',['الكود الموحّد','اسم الطالب','هاتف الطالب','هاتف ولي الأمر','المسار','المجموعة','الشهر','العام الدراسي','الترم','حالة الدفع','ملاحظات'],rows);
    };
    ...
    function mapImportedRow(row){
      const aliases={name:['name','studentname','اسم','اسم الطالب'],studentPhone:['studentphone','هاتف الطالب','رقم الطالب'],parentPhone:['parentphone','هاتف ولي الأمر','رقم ولي الامر','رقم ولي الأمر'],grade:['grade','المسار'],group:['group','المجموعة'],month:['month','الشهر'],academicYear:['academicyear','year','العام الدراسي'],term:['term','semester','الترم'],notes:['notes','ملاحظات']};
    ...
    window.importStudentsFile=async function(input){
      ...
      try{
        ...
        for(let i=0;i<mapped.length;i+=1){
          const schedule=(adminData.groups||[]).find(group=>String(group.name||group.group||'')===String(mapped[i].group||'')&&(typeof adminSameAcademic!=='function'||adminSameAcademic(group.grade,mapped[i].grade)));const item={...mapped[i],studentName:mapped[i].name,groupId:schedule?.id||'',scheduleId:schedule?.id||'',scheduleDays:schedule?.days||'',scheduleStartTime:schedule?.startTime||'',scheduleEndTime:schedule?.endTime||'',paid:false,active:true,attendance:[],grades:[],homeworks:[],recitations:[]};
    ...
    window.editStudent=async function(code){
      ...
      const grade=prompt('المسار التعليمي',s.grade||GRADES[0]);if(grade===null)return;
      ...
      const schedule=(adminData.groups||[]).find(item=>String(item.name||item.group||'')===String(group)&&(typeof adminSameAcademic!=='function'||adminSameAcademic(item.grade,grade)));const before={...s},updated={...s,name,studentName:name,parentPhone,grade,group,groupId:schedule?.id||'',scheduleId:schedule?.id||'',scheduleDays:schedule?.days||'',scheduleStartTime:schedule?.startTime||'',scheduleEndTime:schedule?.endTime||'',groupAssignmentPending:!schedule,academicYear,term:ACADEMIC_TERMS.includes(term)?term:currentAcademicContext().term,notes:prompt('ملاحظات المدرس',s.notes||'')??s.notes??''};
    ...
    renderExams=function(){
      ...
      if(existing&&!document.getElementById('examScheduleSummary')){
        existing.insertAdjacentHTML('beforeend',`<div id="examScheduleSummary" class="exam-schedule-summary"><h3>مواعيد الإتاحة</h3>${adminData.exams.map(ex=>`<div class="mobile-row"><div><b>${safe(ex.title)}</b><small>${safe(ex.academicYear||ctx.academicYear)} · ${safe(ex.term||ctx.term)} · ${safe(ex.group||'كل المجموعات')}</small></div><span class="badge ${examAvailabilityClass(ex)}">${safe(examAvailabilityText(ex))}</span>${ex.pdfUrl?`<a class="small-btn" href="${safe(ex.pdfUrl)}" target="_blank" rel="noopener noreferrer">فتح PDF</a>`:''}</div>`).join('')||'<p class="section-desc">لا توجد اختبارات.</p>'}</div>`);
      }
    ...
    renderSettings=function(){
      fresh();const ctx=currentAcademicContext();content(`<div class="section-head"><div><span class="kicker"><span data-icon="sparkles"></span> الإعدادات</span><h2 class="section-title">إعدادات المنصة والعام الدراسي</h2></div></div><div class="card"><form id="settingsForm" class="grid grid-2"><div class="field"><label>الدومين الأساسي</label><input name="siteUrl" value="${safe(adminData.settings.siteUrl||DEFAULT_SITE_URL||'')}" placeholder="الدومين الأساسي"></div><div class="field"><label>رقم واتساب المدرس</label><input name="teacherPhone" value="${safe(adminData.settings.teacherPhone||TEACHER_WHATSAPP||'')}" placeholder="رقم واتساب المدرس"></div><div class="field"><label>العام الدراسي الحالي</label><select name="academicYear">${academicYearOptions(ctx.academicYear)}</select></div><div class="field"><label>الترم الحالي</label><select name="term">${ACADEMIC_TERMS.map(t=>`<option ${t===ctx.term?'selected':''}>${t}</option>`).join('')}</select></div><div class="field grid-span-full"><label>رسالة تنبيه للطلاب</label><textarea name="homeNotice" placeholder="رسالة تنبيه للطلاب">${safe(adminData.settings.homeNotice||'')}</textarea></div><button class="btn primary grid-span-full" type="submit"><span data-icon="sparkles"></span> حفظ الإعدادات</button></form></div><div class="grid grid-3" style="margin-top:18px"><div class="seo-card"><h3>تخزين منفصل</h3><p>الطلاب والحضور والدرجات والاختبارات تُحفظ في Collections منفصلة، ولا يتم تحديث مستند ضخم واحد.</p></div><div class="seo-card"><h3>أمان النماذج</h3><p>الحجز والتقييم يعملان من خلال Cloud Functions فقط مع Rate Limit.</p></div><div class="seo-card"><h3>المصادقة الثنائية</h3><p>الكود جاهز لإعادة تعيين كلمة المرور. تفعيل MFA نفسه يحتاج تفعيل Identity Platform من Firebase Console.</p></div></div>`);
      document.getElementById('settingsForm').onsubmit=async event=>{event.preventDefault();const form=event.target,button=form.querySelector('[type="submit"]'),values=Object.fromEntries(new FormData(form).entries());if(!ACADEMIC_YEAR_PATTERN.test(values.academicYear))return aToast('صيغة العام الدراسي غير صحيحة');const before={...adminData.settings};adminData.settings={...adminData.settings,...values};button.disabled=true;button.classList.add('is-loading');try{if(!window.MFCloud?.saveSettings)throw new Error('Settings service unavailable');await window.MFCloud.saveSettings(adminData.settings);saveData(adminData);aToast('تم حفظ إعدادات العام الدراسي والمنصة');renderSettings();}catch(error){adminData.settings=before;saveData(adminData);aToast(adminActionErrorMessage(error,'تعذر حفظ إعدادات المنصة.'));}finally{button.disabled=false;button.classList.remove('is-loading');}};
    };

    renderBackup=async function(loading=false){
      fresh();content(`<div class="section-head"><div><span class="kicker"><span data-icon="database"></span> النسخ الاحتياطي</span><h2 class="section-title">حماية بيانات المنصة</h2><p class="section-desc">نسخ يدوية قابلة للاستعادة، ونسخ تلقائية مضغوطة محفوظة في Firebase Storage.</p></div></div><div class="grid grid-3"><div class="card backup-action-card"><h3>نسخة يدوية JSON</h3><p>تنزيل كل البيانات الحالية على جهازك.</p><button type="button" class="btn primary" onclick="exportFullBackup()">تنزيل نسخة</button></div><div class="card backup-action-card"><h3>استعادة نسخة يدوية</h3><p>يتم تنزيل نسخة من الوضع الحالي قبل الاستعادة.</p><label class="btn ghost file-button">اختيار JSON<input type="file" accept="application/json,.json" onchange="importFullBackup(this)" hidden></label></div><div class="card backup-action-card"><h3>نسخة سحابية الآن</h3><p>النسخ التلقائي اليومي يحتاج نشر Scheduled Function وخطة Firebase تدعم Scheduler.</p><button type="button" class="btn dark" onclick="createAutomaticBackupNow()">إنشاء نسخة الآن</button></div></div><div class="grid grid-2" style="margin-top:18px"><div class="card"><div class="section-head mini"><div><h3>النسخ السحابية</h3><p class="section-desc">يتم الاحتفاظ بآخر 14 يومًا افتراضيًا.</p></div><button type="button" class="small-btn" onclick="loadAutomaticBackups()">تحديث</button></div><div id="automaticBackupsBox">${loading?'جاري التحميل...':'اضغط تحديث لعرض النسخ.'}</div></div><div class="card"><div class="section-head mini"><div><h3>سجل العمليات</h3></div><button type="button" class="small-btn" onclick="refreshActivityLog()">تحديث</button></div><div id="activityLogBox">جاري التحميل...</div></div></div>`);
    ...
    window.restoreAutomaticBackup=async function(name){if(!isTeacherManager())return aToast('الاستعادة متاحة للإدارة فقط');try{aToast('جارٍ التحقق من النسخة وتجهيز خطة الاستعادة…');const plan=await window.MFCloud.previewAutomaticBackup(name);if(plan.financialConflictCount)throw new Error(`النسخة تتعارض مع ${plan.financialConflictCount} سجل مالي حالي. يلزم استرداد معزول ومراجعة الخزنة.`);const typed=prompt(`استعادة دمج: ${plan.writes} مستند، دون حذف مستندات غائبة. ${plan.preserved.length} مجموعة غير مضمنة ستبقى كما هي. ملفات Storage غير مضمنة. أوقف تعديل البيانات أثناء التنفيذ؛ العملية على دفعات وقد تحتاج استكمالًا عند الفشل. ستنشأ نسخة أمان. اكتب: استعادة`);if(typed!=='استعادة')return;const result=await window.MFCloud.restoreAutomaticBackup(name,plan.planId);if(!result?.ok)throw new Error('تعذر تأكيد الاستعادة');await reloadFromCloud();aToast('اكتملت استعادة الدمج وحُفظت نسخة الأمان');renderBackup();}catch(error){aToast(adminActionErrorMessage(error,'تعذرت الاستعادة؛ راجع سجل التنفيذ ونسخة الأمان قبل إعادة المحاولة.'));}};

    async function renderClientErrors(){
      if(!isTeacherManager())return content('<div class="card"><h2>غير مصرح</h2></div>');
      content(`<div class="section-head"><div><span class="kicker"><span data-icon="database"></span> مراقبة الأخطاء</span><h2 class="section-title">أخطاء أجهزة المستخدمين</h2><p class="section-desc">آخر أخطاء JavaScript التي أرسلها الموقع بدون بيانات حساسة.</p></div><div class="mobile-actions"><button type="button" class="btn ghost" onclick="renderSection()">تحديث</button><button type="button" class="btn danger" onclick="clearClientErrors()">مسح الكل</button></div></div><div class="card" id="clientErrorsBox"><div class="skeleton" style="height:150px"></div></div>`);
      try{const rows=await window.MFCloud?.getClientErrors?.(100)||[];const box=document.getElementById('clientErrorsBox');box.innerHTML=rows.length?rows.map(row=>`<article class="error-log-card"><div><b>${safe(row.message||'خطأ')}</b><small>${safe(row.page||'')}</small><small>${safe(row.createdAt?.toDate?row.createdAt.toDate().toLocaleString('ar-EG'):row.createdAt||'')}</small></div><button type="button" class="small-btn danger" onclick="deleteClientError('${safe(row.id)}')">حذف</button></article>`).join(''):'<div class="empty-state"><h3>لا توجد أخطاء مسجلة</h3><p>هذا مؤشر جيد.</p></div>';}catch(error){document.getElementById('clientErrorsBox').innerHTML='<p class="section-desc">تعذر تحميل الأخطاء.</p>';}
    }
    ...
    window.approveReview=async function(id){const review=adminData.reviews.find(item=>String(item.id)===String(id));if(!review)return;const previous=review.approved;review.approved=true;try{if(!window.MFCloud?.saveContent)throw new Error('Content service unavailable');await window.MFCloud.saveContent('reviews',review);saveData(adminData);aToast('تم نشر التقييم على الموقع');renderReviewsAdmin();}catch(error){review.approved=previous;saveData(adminData);aToast(adminActionErrorMessage(error,'تعذر نشر التقييم.'));}};
  ...
  const studentHelpItems=[
    ...
    {q:'إيه المسارات المتاحة؟',keys:'مسارات برمجة بايثون ذكاء اصطناعي ثانوي مبتدئين',a:'متاح أولى ثانوي بكالوريا، وتانية ثانوي بكالوريا، وأساسيات برمجة.'},
    ...
    {q:'ولي الأمر يتابع إزاي؟',keys:'ولي الامر الاب الام متابعة تقرير',a:'يستخدم ولي الأمر نفس كود الطالب الموحّد أو يمسح نفس الباركود لعرض الحضور والتطبيق العملي والدرجات والواجبات والملاحظات.',link:'parent.html',label:'فتح صفحة ولي الأمر'},
    ...
    {q:'أشوف الدرجات والنتائج فين؟',keys:'درجة درجات نتيجة نتائج امتحان',a:'تظهر الدرجات في صفحة الطالب وولي الأمر، ونتائج الاختبارات تظهر بعد التسليم أو بعد تصحيح الأسئلة المقالية.'},
    {q:'أدخل الامتحان إزاي؟',keys:'امتحان اختبار ابدأ',a:'افتح صفحة الاختبارات، اكتب كود الطالب، ثم اختر الامتحان المتاح لمسارك واضغط بدء.',link:'exams.html',label:'فتح الاختبارات'},
    ...
    {q:'الامتحان مش ظاهر',keys:'امتحان مش ظاهر غير موجود',a:'تأكد من كود الطالب وموعد فتح الامتحان وأنه مخصص لمسارك ومجموعتك. بعد ذلك حدّث الصفحة.'},
    ...
    {q:'ألاقي التدريبات والأسئلة فين؟',keys:'مراجعة تدريبات ملفات مذكرة اسئلة بنك',a:'التدريبات والتحديات البرمجية متاحان من صفحات المحتوى، ويمكنك تصفية المحتوى حسب المسار.',link:'materials.html',label:'فتح التدريبات'},
    {q:'أعرف حالة الدفع إزاي؟',keys:'دفع دفعت فلوس مصاريف حالة الدفع',a:'حالة الدفع تظهر داخل تقرير الطالب وولي الأمر. الدفع يتم داخل الأكاديمية فقط ولا يوجد دفع إلكتروني بالموقع.'},
    ...
    {q:'هل أقدر أغيّر بياناتي؟',keys:'تعديل تغيير اسم رقم صف مجموعة بيانات',a:'لحماية بيانات الطالب، تعديل الاسم أو الرقم أو المسار أو المجموعة يتم من خلال المدرس.'},
  ...
  function initStudentHelp(){
    ...
    document.body.insertAdjacentHTML('beforeend',`<button id="studentHelpButton" class="student-help-button robot-faq-btn" type="button" aria-label="افتح مساعد Techno Minds" title="مساعد Techno Minds" aria-expanded="false"><span class="tm-robot-icon" aria-hidden="true"><svg viewBox="0 0 48 48" fill="none"><path d="M24 8V4m-3 0h6"/><rect x="8" y="11" width="32" height="28" rx="10"/><path d="M8 23H4v9h4m32-9h4v9h-4M16 39v5m16-5v5"/><circle cx="18" cy="24" r="3" fill="currentColor" stroke="none"/><circle cx="30" cy="24" r="3" fill="currentColor" stroke="none"/><path d="M17 32c4 3 10 3 14 0"/></svg></span><span class="tm-robot-pulse"></span></button><section id="studentHelpPanel" class="student-help-panel" hidden aria-label="مساعد Techno Minds"><header><div><b>روبوت Techno Minds</b><small>اسأل عن م. عمرو أو المنصة أو خدمات الطالب</small></div><button type="button" id="studentHelpClose" aria-label="إغلاق">×</button></header><form id="studentHelpSearch"><input name="question" autocomplete="off" placeholder="مثال: أشغّل الكود منين؟" aria-label="اكتب سؤالك"><button type="submit">إرسال</button></form><div id="studentHelpAnswer" aria-live="polite"><p>أقدر أساعدك في التعرف على م. عمرو، المسار، الحجز، المعمل العملي، بوابات المتابعة والاختبارات.</p></div><div class="student-help-questions">${studentHelpItems.map((item,index)=>`<button type="button" data-help-index="${index}">${safeText(item.q)}</button>`).join('')}</div></section>`);
  ...
  function usePaymentWording(root=document){
    const replacements=[['المدفوعات','الدفع'],['حالة الدفع','الدفع'],['حالة الدفع','حالة الدفع'],['تم الاشتراك','تم الدفع'],['غير مشترك','لم يتم الدفع']];
