(function(){
  ...
  function renderExamsV6061(){
    ...
    const examCards=(adminData.exams||[]).slice().reverse().map(exam=>{const [label,badge]=examStatus(exam);return `<article class="admin-exam-card"><div class="admin-exam-card-head"><span class="iconbox" data-icon="clipboard"></span><span class="badge ${badge}">${label}</span></div><h3>${safe(exam.title)}</h3><p>${safe(exam.grade||'كل المسارات')} · ${safe(exam.group||'كل المجموعات')} · ${safe(exam.duration||20)} دقيقة</p><div class="admin-exam-meta"><span>${safe(exam.questionCount||0)} سؤال</span><span>من ${safe(exam.totalScore||exam.maxScore||exam.questionCount||100)} درجة</span></div><div class="admin-exam-actions">${exam.pdfUrl?`<a class="small-btn" href="${safe(exam.pdfUrl)}" target="_blank" rel="noopener noreferrer">PDF</a>`:''}<button class="small-btn primary" type="button" onclick="editLiveExam('${safe(exam.id)}')">تعديل</button><button class="small-btn" type="button" onclick="toggleLiveExam('${safe(exam.id)}')">${exam.active===false?'تفعيل':'إيقاف'}</button><button class="small-btn danger" type="button" onclick="deleteItem('exams','${safe(exam.id)}')">حذف</button></div></article>`;}).join('');
    content(`<div class="section-head compact-admin-head"><div><span class="kicker"><span data-icon="clipboard"></span> الاختبارات</span><h2 class="section-title">إدارة الاختبارات</h2><p class="section-desc">إنشاء الامتحان، متابعة المحاولات، والتصحيح من شاشة واحدة منظمة.</p></div><button class="btn primary" type="button" onclick="toggleExamCreator()"><span data-icon="plus"></span> إضافة اختبار</button></div>
      ...
      <details class="card admin-collapsible" open><summary>مين امتحن ومين لسه <span class="badge">${attempts.length}</span></summary><div class="admin-detail-body"><div class="exam-attendance-filters"><label>الامتحان<select id="examAttendanceExam"><option value="">اختر امتحانًا</option>${(adminData.exams||[]).map(row=>`<option value="${safe(row.id)}">${safe(row.title)}</option>`).join('')}</select></label><label>الصف<select id="examAttendanceGrade"><option value="">كل الصفوف</option>${grades()}</select></label><label>المجموعة<select id="examAttendanceGroup"><option value="">كل المجموعات</option>${(adminData.groups||[]).map(group=>`<option>${safe(group.name||group.group||'')}</option>`).join('')}</select></label></div><div id="examAttendanceRows">${empty('اختر الامتحان لعرض الطلاب.')}</div></div></details>
    ...
    ['examAttendanceExam','examAttendanceGrade','examAttendanceGroup'].forEach(id=>document.getElementById(id)?.addEventListener('change',renderExamAttendanceRows));
    hydrateIcons();
  }

  function renderExamAttendanceRows(){const examId=document.getElementById('examAttendanceExam')?.value||'',grade=document.getElementById('examAttendanceGrade')?.value||'',group=document.getElementById('examAttendanceGroup')?.value||'',box=document.getElementById('examAttendanceRows');if(!box)return;const exam=adminData.exams.find(row=>String(row.id)===examId);if(!exam){box.innerHTML=empty('اختر الامتحان لعرض الطلاب.');return;}const students=(adminData.students||[]).filter(student=>(!grade||String(student.grade||student.track||'')===grade)&&(!group||String(student.group||'')===group)&&(exam.grade==='كل المسارات'||!exam.grade||String(student.grade||student.track||'')===String(exam.grade))&&(!exam.group||String(student.group||'')===String(exam.group)));const attempts=adminData.examAttempts||[];box.innerHTML=`<div class="exam-attendance-summary"><span class="badge good">امتحن ${students.filter(student=>attempts.some(row=>String(row.examId)===examId&&String(row.studentCode)===String(student.studentCode||student.code))).length}</span><span class="badge warn">لسه ${students.filter(student=>!attempts.some(row=>String(row.examId)===examId&&String(row.studentCode)===String(student.studentCode||student.code))).length}</span></div>${students.map(student=>{const code=student.studentCode||student.code,attempt=attempts.find(row=>String(row.examId)===examId&&String(row.studentCode)===String(code));return `<div class="mobile-row"><div><b>${safe(student.name||student.studentName||'طالب')}</b><small>${safe(code||'')} · ${safe(student.grade||'')} · ${safe(student.group||'')}</small></div><span class="badge ${attempt?'good':'warn'}">${attempt?examAttemptScoreText(attempt):'لم يمتحن'}</span></div>`;}).join('')||empty('لا يوجد طلاب مطابقون للفلاتر.')}`;}
  ...
  window.toggleLiveExam=async function(id){const exam=adminData.exams.find(row=>String(row.id)===String(id));if(!exam)return;exam.active=exam.active===false;await saveAdminDataNow();aToast(exam.active?'تم تفعيل الامتحان':'تم إيقاف الامتحان');renderExamsV6061();};
  window.editLiveExam=async function(id){const exam=adminData.exams.find(row=>String(row.id)===String(id));if(!exam)return;const title=prompt('اسم الامتحان',exam.title||'');if(title===null)return;const duration=prompt('المدة بالدقائق',exam.duration||20);if(duration===null)return;const closeAt=prompt('موعد الإغلاق بصيغة 2026-08-01T22:00 (اتركه فارغًا بدون إغلاق)',String(exam.closeAt||'').slice(0,16));if(closeAt===null)return;exam.title=String(title).trim()||exam.title;exam.duration=Math.max(1,Math.min(300,Number(duration)||20));exam.closeAt=String(closeAt).trim();exam.updatedAt=new Date().toISOString();await saveAdminDataNow();aToast('تم تعديل الامتحان وهو ما زال متاحًا حسب موعده');renderExamsV6061();};
  ...
  async function saveAssignmentV6061(event){
    ...
    const type=values.type,choices=type==='truefalse'?['صح','غلط']:[values.choice0,values.choice1,values.choice2,values.choice3].map(value=>String(value||'').trim()).filter(Boolean);
    ...
    try{
      ...
      assignment={id:`hw-${Date.now()}`,title:String(values.title||'').trim(),grade:String(values.grade||'').trim(),type,description:String(values.description||'').trim(),dueDate:values.dueDate||'',choices:type==='mcq'||type==='truefalse'?choices:[],correctIndex:type==='mcq'||type==='truefalse'?Number(values.correctIndex):null,language:type==='code'?values.language:'',starterCode:type==='code'?values.starterCode:'',active:true,published:true,createdAt:new Date().toISOString()};
