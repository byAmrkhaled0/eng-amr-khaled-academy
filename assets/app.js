...
function setupUnifiedHeader(){
  ...
  const links=[
    ...
    ['student.html','بوابة الطالب'],['parent.html','ولي الأمر'],['materials.html','المحاضرات'],['theory-lectures.html','محاضرات النظري'],['questions.html','الأسئلة'],['exams.html','الاختبارات']
  ];
...
function bindStudentDashboard(){
  ...
  buttons.forEach(btn=>btn.addEventListener('click',async()=>{
    ...
    if(key==='motivation'&&target&&!target.dataset.rankingLoaded&&lastPortalStudent&&window.MFCloud?.getStudentLeaderboardPosition){
      ...
      try{const result=await window.MFCloud.getStudentLeaderboardPosition(lastPortalStudent.studentCode);target.dataset.rankingLoaded='true';if(ranking){const bonus=Number(result.motivationBonus||0),rank=result.rank;ranking.innerHTML=`<div><small>مركزك هذا الشهر</small><b>${rank?`رقم ${esc(rank)}`:'لم تدخل الترتيب بعد'}</b><span>${rank?`من ${esc(result.totalStudents)} طالب في مسارك`:'ابدأ نشاطك لتحصل على مركز'}</span></div><div><small>تقييم الترتيب</small><b>${esc(result.score||0)}%</b><span>بونص التحفيز ${bonus>=0?'+':''}${esc(bonus)}</span></div>`;}}
...
async function renderPublicLeaderboard(force=false){
  ...
  box.innerHTML=rows.length?rows.map((x,i)=>{
    ...
    return `<article class="leaderboard-row rank-${i+1}">
      ...
      <div class="leaderboard-student-info">
        ...
        <span class="leaderboard-grade">${esc(x.grade||'المسار غير محدد')}</span>
  ...
  }).join(''):`<div class="empty-state compact-empty-v29"><span class="iconbox" data-icon="star"></span><h3>لا يوجد ترتيب لصف ${esc(selectedGrade)} بعد</h3><p>سيظهر أفضل خمسة طلاب من هذا المسار تلقائيًا بعد تسجيل نشاطهم خلال الشهر الحالي.</p></div>`;
  hydrateIcons();
}
...
function renderUnifiedResourcesPage(){
  ...
  const student=currentStudentResources.student||{},resourceMode=document.body.dataset.resourceMode||'materials',allMaterials=sortStudentResources(currentStudentResources.materials),materials=allMaterials.filter(row=>resourceMode==='theory'?row.lectureCategory==='theory':row.lectureCategory!=='theory'),questions=sortStudentResources(currentStudentResources.questions);
...
window.startExam=async function(examId,studentCode){
  ...
  let current=Math.min(Math.max(Number(draft.current||0),0),qs.length-1),finished=false,timer=null,serverSaveTimer=null,lastServerSnapshot='',autoSubmitTriggered=false;
  ...
  const closeExamShell=()=>{clearInterval(timer);clearTimeout(timer);clearTimeout(serverSaveTimer);window.removeEventListener('pagehide',pageHideSave);window.removeEventListener('beforeunload',unloadGuard);window.__tmExamInProgress=false;overlay.classList.remove('show');document.body.classList.remove('exam-open');};
