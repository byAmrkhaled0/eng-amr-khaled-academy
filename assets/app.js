...
function calcStudent(st){
  ...
  const graded=[...(st.grades||[]),...(st.examAttempts||[])].filter(g=>g.score!==''&&g.score!==undefined&&g.score!==null&&!isNaN(Number(g.score)));
...
function studentAssignmentCard(assignment,studentCode,isParent=false,submission=null){
  ...
  const closed=assignment.submissionClosed===true;
  ...
  const form=!isParent&&type!=='file'&&!closed?`<form class="assignment-answer-form" data-student-code="${esc(studentCode)}" data-assignment-id="${esc(assignment.id)}" data-assignment-type="${esc(type)}">${answer}<div class="assignment-submit-row"><span class="assignment-form-state" aria-live="polite">${submission?'تم التسليم من قبل — يمكنك التعديل وإعادة التسليم.':''}</span><button class="btn primary" type="submit"><span data-icon="upload"></span> ${submission?'إعادة التسليم':'تسليم الواجب'}</button></div></form>`:'';
  const result=submission&&submission.answerType==='multi'?`<details class="homework-submission-result"><summary>${submission.needsManualReview?'التسليم ينتظر تصحيح المدرس':`النتيجة: ${esc(submission.score??submission.autoScore??0)} من ${esc(submission.maxScore||assignment.totalScore||1)}`}</summary>${(submission.answers||[]).map((row,index)=>`<div class="exam-review-item ${row.correct===true?'correct':row.correct===false?'wrong':'pending'}"><b>${index+1}. ${esc(row.question)}</b><span>${row.correct===true?'✓ صحيحة':row.correct===false?'✕ خاطئة':'بانتظار التصحيح'} · ${esc(row.awardedMark??0)} من ${esc(row.mark||1)}</span></div>`).join('')}</details>`:'';
  return `<article class="student-assignment-card"><div class="student-assignment-head"><div><span class="record-eyebrow">${typeName}${assignment.lessonNumber?` · الحصة ${esc(assignment.lessonNumber)}`:''}</span><h4>${esc(assignment.title||'واجب جديد')}</h4><small>${esc(assignment.lessonTitle||'')}</small></div><span class="badge ${submission?'good':closed?'danger':'warn'}">${submission?'تم التسليم':closed?'انتهى التسليم':'مطلوب'}</span></div><p>${esc(assignment.description||'')}</p><small>${esc(assignment.questionCount||1)} سؤال · من ${esc(assignment.totalScore||1)} درجة · ${due}</small>${download}${result}${form}${isParent?'<p class="assignment-parent-note">يستطيع الطالب تسليم الواجب من بوابة الطالب.</p>':''}</article>`;
}
function studentProfileHTML(raw, isParent=false){
  ...
  return `<div class="student-app-dashboard">
    ...
    <section class="student-kpi-grid">
      ...
      <article><span data-icon="book-open"></span><b>${c.recitationPct}%</b><small>انتظام التطبيق العملي</small></article>
    ...
    <section class="student-tab-panel show" data-student-panel="overview">
      <div class="student-overview-grid">
        <article class="student-highlight-card"><span class="iconbox" data-icon="sparkles"></span><div><small>تقييمك الحالي</small><h3>${esc(c.level)}</h3><p>استمر في الحضور وحل الاختبارات والتطبيق العملي والواجبات لتحسين ترتيبك.</p></div></article>
        <article class="student-highlight-card"><span class="iconbox" data-icon="star"></span><div><small>آخر درجة</small><h3>${c.lastGrade?`${esc(c.lastGrade.score)} من ${esc(c.lastGrade.maxScore||100)}`:'لا توجد بعد'}</h3><p>${c.lastGrade?esc(c.lastGrade.exam||c.lastGrade.examTitle||'آخر امتحان'):'ستظهر آخر نتيجة هنا.'}</p></div></article>
      </div>
    ...
    <section class="student-tab-panel" data-student-panel="homework"><div class="student-panel-title"><div><span class="kicker"><span data-icon="file-text"></span> الواجبات</span><h3>الواجب المنشور لصفك</h3></div><span class="badge warn">${assignments.length} واجب</span></div><div class="student-assignment-list">${assignmentCards}</div><details class="student-homework-history"><summary>سجل الواجبات السابقة</summary><div class="student-record-list">${homeworkCards}</div></details></section>
...
function renderExamPortal(st,exams){
  ...
  const resultCards=attempts.length?attempts.map(a=>{const ready=a.score!==null&&a.score!==undefined&&a.score!=='';const max=Number(a.maxScore||100),review=Array.isArray(a.review)?a.review:[];return `<article class="exam-result-card"><div><span class="record-eyebrow">${esc(formatPortalDate(a.submittedAt))}</span><h4>${esc(a.examTitle||'امتحان')}</h4><small>${a.needsManualReview?'ينتظر تصحيح الأسئلة المقالية':'تم التصحيح الآمن على الخادم'}</small>${review.length?`<details class="exam-answer-review"><summary>عرض التصحيح التفصيلي</summary>${review.map((row,index)=>`<div class="exam-review-item ${row.correct===true?'correct':row.correct===false?'wrong':'pending'}"><b>${index+1}. ${esc(row.question)}</b><span>${row.correct===true?'✓ صحيحة':row.correct===false?'✕ خاطئة':'بانتظار المدرس'} · ${esc(row.awardedMark??0)} من ${esc(row.mark||1)}</span><small>إجابتك: ${esc(row.answer||'لم يجب')}</small>${row.correct===false?`<small>الصحيح: ${esc(row.correctAnswer||'-')}</small>`:''}</div>`).join('')}</details>`:''}</div><strong class="score-pill ${ready?scoreClass((Number(a.score)/max)*100):'warn'}">${ready?`${esc(a.score)} من ${esc(max)}`:'قيد التصحيح'}</strong></article>`;}).join(''):'<div class="portal-empty"><span class="iconbox" data-icon="bar-chart"></span><h3>لا توجد محاولات بعد</h3><p>ستظهر نتائجك هنا بعد التسليم.</p></div>';
