  ...
  return `<div class="parent-monthly-report-v70" id="parentMonthlyReport">
    ...
    <article class="parent-report-sheet-v70 parent-report-sheet-v71">
      ...
      <div class="parent-report-content-grid-v71">
        <section class="parent-report-section-v70 academic"><div class="parent-report-section-head-v70"><h3>الامتحانات والدرجات</h3><p>سلّم ${esc(results.submittedExams||0)} من ${esc(results.requiredExams||0)} · متوسط المصحح ${score(results.average)}</p></div><div class="parent-report-list-v70">${examRows.map(row=>`<div><span><b>${esc(row.activityName||row.examTitle||'امتحان')}</b><small>${esc(formatPortalDate(row.date||row.submittedAt))} · ${esc(row.assessmentMode==='paper'?'ورقي':'إلكتروني')} · ${esc(row.status==='started'?'بدأ ولم يسلّم':row.status==='absent'?'غائب':row.status==='pending_review'?'قيد التصحيح':'تم التصحيح')}</small></span><strong>${esc(monthlyResultStatus(row))}</strong></div>`).join('')||'<p>لا يوجد نشاط امتحانات مسجل في هذا الشهر.</p>'}${moreExams?`<small class="parent-report-more-v70">+ ${moreExams} امتحانات أخرى</small>`:''}</div></section>
...
function parentMonthlyReportHTML(report){
  ...
  const feedback=parentReportFeedback(report),stableNote='البيانات المتاحة لا تسجل ملاحظة إضافية.',recommendation=feedback.plan||'لا توجد توصية محددة مدعومة ببيانات هذا الشهر.';
  return `<div class="parent-monthly-report-v70" id="parentMonthlyReport">
    ...
    <article class="parent-report-sheet-v70 parent-report-sheet-v71 parent-report-sheet-v72">
      ...
      <section class="parent-report-feedback-v7002" aria-label="تقييم حالة الطالب هذا الشهر"><h3>تقييم حالة الطالب هذا الشهر</h3><p>${esc(feedback.summary)}</p>${feedback.strengths.length?`<div><b>نقاط القوة:</b> ${feedback.strengths.map(esc).join(' · ')}</div>`:''}${feedback.focus.length?`<div><b>يحتاج تركيز:</b> ${feedback.focus.map(esc).join(' · ')}</div>`:''}${feedback.plan?`<div><b>خطة الشهر القادم:</b> ${esc(feedback.plan)}</div>`:''}${feedback.teacherNote?`<div><b>ملاحظة المعلم:</b> ${esc(feedback.teacherNote)}</div>`:''}</section>
      <section class="parent-report-kpis-v70 parent-report-kpis-v71" aria-label="ملخص مؤشرات الشهر">
      ...
      <div class="parent-report-content-grid-v71 parent-report-activity-grid-v72">
        <section class="parent-report-section-v70 academic"><div class="parent-report-section-head-v70"><h3>الامتحانات والدرجات</h3><p>${esc(parentReportCount(results.submittedExams,results.requiredExams,'تم تسليمها'))} · متوسط ${score(results.average)}</p></div><div class="parent-report-list-v70">${examRows.map(row=>`<div><span><b>${esc(row.activityName||row.examTitle||'امتحان')}</b><small>${esc(formatPortalDate(row.date||row.submittedAt))} · ${esc(row.assessmentMode==='paper'?'ورقي':'إلكتروني')} · ${esc(row.status==='started'?'بدأ ولم يسلّم':row.status==='absent'?'غائب عن الامتحان':row.status==='pending_review'?'قيد التصحيح':'تم التصحيح')}</small></span><strong>${esc(monthlyResultStatus(row))}</strong></div>`).join('')||'<p>لا يوجد نشاط امتحانات مسجل في هذا الشهر.</p>'}</div></section>
      ...
      <section class="parent-report-section-v70 parent-report-followup-v70 parent-report-followup-v71"><h3>ملخص المتابعة وخطة ولي الأمر</h3><div><p class="strength"><b>نقطة قوة:</b> ${esc(feedback.strengths[0]||'لا توجد نقطة قوة محددة في البيانات المتاحة.')}</p><p class="concern"><b>يحتاج متابعة:</b> ${esc(feedback.focus[0]||stableNote)}</p><p class="action"><b>الخطوة المقترحة:</b> ${esc(recommendation)}</p><p class="teacher"><b>ملاحظة المدرس:</b> ${esc(report.teacherNotes||'لا توجد ملاحظة إضافية.')}</p></div></section>
...
function parentReportFeedback(report){
  const attendance=report?.attendance||{},results=report?.results||{},homework=report?.homework||{},practical=report?.practical||{},study=report?.study||{};
  const pct=value=>value===null||value===undefined||!Number.isFinite(Number(value))?null:Number(value);
  const overall=pct(report?.overallScore),att=pct(attendance.percentage),exam=pct(results.average),hwGrade=pct(homework.averageGrade);
  const hasEvidence=[overall,att,exam,hwGrade].some(value=>value!==null)||Number(homework.required||0)>0||Number(practical.count||0)>0||Number(study.lecturesAvailable||0)>0;
  if(!hasEvidence)return {summary:'البيانات المتاحة هذا الشهر غير كافية لتقييم كامل.',strengths:[],focus:[],plan:'',teacherNote:String(report?.teacherNotes||''),insufficient:true};
  const strengths=[...(report.strengths||[])],focus=[...(report.concerns||[])],recommendations=[...(report.recommendations||[])];
  const add=(array,value)=>{if(value&&!array.includes(value))array.push(value);};
  if(att!==null){if(att>=95)add(strengths,'ملتزم جدًا بالحضور.');else if(att<85){add(focus,'الحضور يحتاج متابعة.');add(recommendations,'راجع مواعيد الحصص القادمة وخطة تعويض الغياب مع المعلم.');}}
  if(attendance.consecutiveAbsenceWarning?.active)add(focus,'الغياب المتكرر يحتاج متابعة.');
  if(exam!==null){if(exam>=85)add(strengths,'نتائج الامتحانات قوية.');else if(exam>=60)add(focus,'المستوى جيد ويحتاج تثبيت بالمراجعة.');else{add(focus,'يحتاج مراجعة المفاهيم الأساسية والتدريب قبل الامتحان.');add(recommendations,'راجع الأسئلة الصعبة وتدرّب على نموذج قصير قبل الامتحان التالي.');}}
  if(Number(homework.missing||0)>0){add(focus,`يوجد ${homework.missing} واجب مستحق لم يتم تسليمه.`);add(recommendations,'استكمل الواجبات الناقصة قبل موعد الحصة القادمة.');}
  if(hwGrade!==null&&hwGrade>=85)add(strengths,'حل الواجبات بمستوى جيد.');
  if(Number(practical.count||0)>0&&Number(practical.completed||0)<Number(practical.count)/2){add(focus,'يحتاج زيادة التطبيق العملي.');add(recommendations,'خصص تدريبًا عمليًا قصيرًا على المهارات التي لم تكتمل.');}
  if(Number(study.lecturesAvailable||0)>0&&Number(study.lecturesCompleted||0)<Number(study.lecturesAvailable)/2){add(focus,'المحاضرات المتاحة تحتاج متابعة.');add(recommendations,'حدد موعدًا لمتابعة المحاضرات غير المكتملة.');}
  const rows=(results.rows||[]).filter(row=>row.status==='graded'&&pct(row.percentage)!==null),paper=rows.filter(row=>row.assessmentMode==='paper'),online=rows.filter(row=>row.assessmentMode!=='paper');
  if(paper.length&&online.length){const avg=items=>items.reduce((sum,row)=>sum+Number(row.percentage),0)/items.length;if(avg(online)-avg(paper)>=15)add(focus,'نتائج الامتحان الورقي أقل من الإلكتروني؛ يحتاج تدريبًا على الحل المستقل.');}
  const trend=parentReportTrend(report);if(trend.status==='improved')add(strengths,trend.detail);if(trend.status==='declined')add(focus,trend.detail);
  const summary=overall===null?`${report?.student?.name||'الطالب'} لديه مؤشرات هذا الشهر تحتاج استكمال البيانات قبل تقييم المستوى العام.`:`${report?.student?.name||'الطالب'} حقق مستوى ${report.level||'قابل للمتابعة'} هذا الشهر بنسبة ${overall}%${att>=95?'، مع التزام قوي بالحضور':exam>=85?'، ونتائج قوية في الامتحانات':''}.`;
  return {summary,strengths:strengths.slice(0,2),focus:focus.slice(0,2),plan:recommendations[0]||'',teacherNote:String(report.teacherNotes||''),insufficient:false};
}
window.parentReportFeedback=parentReportFeedback;
function parentReportWhatsAppIntro(report,firstDelivery=false){
  const st=report?.student||{},portal='https://eng-amr-khaled-academy.vercel.app/parent.html',feedback=parentReportFeedback(report);
  const score=report?.overallScore===null||report?.overallScore===undefined?'غير محسوب':`${report.overallScore}%`;
  const details=[`${firstDelivery?'حالة الطالب':'المستوى العام'}: ${report?.level||'بيانات غير كافية'} — ${score}`,
    feedback.strengths[0]?`نقطة القوة: ${feedback.strengths[0]}`:'',feedback.focus[0]?`يحتاج متابعة: ${feedback.focus[0]}`:'',feedback.plan?`التركيز الفترة القادمة: ${feedback.plan}`:'',feedback.teacherNote?`ملاحظة المعلم: ${feedback.teacherNote}`:''].filter(Boolean).join('\n');
  if(!firstDelivery)return `السلام عليكم،
مرفق تقرير ${st.name||'-'} عن شهر ${reportMonthLabel(report.monthKey)}، وصورة التقرير كاملة.
...
تقدر تتابع مستوى ابنك في أي وقت من بوابة ولي الأمر:
${portal}
كود الطالب الموحد: ${st.studentCode||'-'}
الكود ده تستخدمه لعرض الحضور والواجبات والامتحانات والتقدم.
...
async function parentReportImageBlob(report){
  ...
  const headerHeight=220,overviewHeight=122,feedbackHeight=232,metricsHeight=202,factsHeight=122,followupHeight=272,footerHeight=54;
  const height=margin+headerHeight+18+overviewHeight+18+feedbackHeight+18+metricsHeight+18+activityHeight+18+attendanceHeight+18+factsHeight+18+followupHeight+footerHeight;
  ...
  y+=overviewHeight+18;const feedback=parentReportFeedback(report);card(margin,y,width-2*margin,feedbackHeight,'#eff8fb','#a9d5e4');write('تقييم حالة الطالب هذا الشهر',width-margin-20,y+30,23,'#0c3151',900);wrap(feedback.summary,width-margin-20,y+63,width-2*margin-42,26,2,18);if(feedback.strengths.length)wrap(`نقاط القوة: ${feedback.strengths.join(' · ')}`,width-margin-20,y+112,width-2*margin-42,25,1,17,'#237351');if(feedback.focus.length)wrap(`يحتاج تركيز: ${feedback.focus.join(' · ')}`,width-margin-20,y+143,width-2*margin-42,25,1,17,'#9a6020');if(feedback.plan)wrap(`خطة الشهر القادم: ${feedback.plan}`,width-margin-20,y+174,width-2*margin-42,25,1,17,'#245b86');if(feedback.teacherNote)wrap(`ملاحظة المعلم: ${feedback.teacherNote}`,width-margin-20,y+204,width-2*margin-42,25,1,17,'#435b72');
  y+=feedbackHeight+18;const metrics=[['الحضور والانضباط',score(attendance.percentage)],['متوسط الامتحانات',score(results.average)],['درجات الواجب',score(homework.averageGrade)],['تسليم الواجبات',parentReportCount(homework.submitted,homework.required)],['المحاضرات المكتملة',parentReportCount(study.lecturesCompleted,study.lecturesAvailable)],['التطبيق العملي',`${practical.completed||0} مكتمل`],['ترتيب المجموعة',motivation.groupRank?`${motivation.groupRank} من ${motivation.groupTotalStudents||'-'}`:'غير متاح'],['نقاط التحفيز',`${Number(motivation.totalPoints||0)>0?'+':''}${motivation.totalPoints??0}`]],tileW=(width-2*margin-gap*3)/4,tileH=94;
  ...
  y+=factsHeight+18;card(margin,y,width-2*margin,followupHeight,'#fffdf8','#ead79e');write('ملخص المتابعة وخطة ولي الأمر',width-margin-20,y+31,24,'#473a20',900);wrap(`نقطة القوة: ${feedback.strengths[0]||'لا توجد نقطة قوة محددة في البيانات المتاحة.'}`,width-margin-20,y+72,width-2*margin-40,27,2,18,'#237351',700);wrap(`يحتاج متابعة: ${feedback.focus[0]||'البيانات المتاحة لا تسجل ملاحظة إضافية.'}`,width-margin-20,y+132,width-2*margin-40,27,2,18,'#9a6020',700);wrap(`الخطوة المقترحة: ${feedback.plan||'لا توجد توصية محددة مدعومة ببيانات هذا الشهر.'}`,width-margin-20,y+192,width-2*margin-40,27,2,18,'#245b86',700);wrap(`ملاحظة المدرس: ${report.teacherNotes||'لا توجد ملاحظة إضافية.'}`,width-margin-20,y+246,width-2*margin-40,27,1,17,'#435b72',700);
...
window.reserveParentWhatsAppWindow=function(parentPhone){
  if(!confirm(`سيتم فتح التقرير على رقم ولي الأمر:\n${parentPhone}\n\nهل تريد المتابعة؟`))return {confirmed:false,popup:null};
  // Reserve browser activation before the report fetch or PNG generation awaits.
  const popup=window.open('about:blank','_blank');if(popup)popup.opener=null;
  return {confirmed:true,popup};
};
window.deliverParentMonthlyReport=async function(report,parentPhone,notify=toast,options={}){
  ...
  const reserved=options.reservedWindow||window.reserveParentWhatsAppWindow(phone);
  if(!reserved.confirmed)return false;
  try{
    ...
    notify('جاري إنشاء صورة التقرير…');
    const blob=await parentReportImageBlob(report),fileName=`Techno-Minds-${name}-${report.monthKey||'report'}.png`,message=parentReportWhatsAppIntro(report,options.firstDelivery===true),file=typeof File==='function'?new File([blob],fileName,{type:'image/png'}):null;
    if(file&&navigator.share&&navigator.canShare?.({files:[file]})){
      try{await navigator.share({title:`تقرير ${report?.student?.name||'الطالب'}`,text:message,files:[file]});reserved.popup?.close();window.parentReportDeliveryMode='system-share-opened';notify('تم فتح مشاركة صورة التقرير والنص. تأكد من إرسالهما داخل التطبيق المختار.');return true;}
      catch(error){if(error?.name==='AbortError'){reserved.popup?.close();return false;}/* Fall back to the already reserved desktop window. */}
    }
    const url=URL.createObjectURL(blob),download=document.createElement('a');download.href=url;download.download=fileName;document.body.appendChild(download);download.click();download.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);
    try{await navigator.clipboard.writeText(message);}catch(_){}
    const opened=reserved.popup&&!reserved.popup.closed?reserved.popup:null;
    if(opened){opened.location.href=whatsappLink(phone,message);window.parentReportDeliveryMode='whatsapp-opened';notify('تم فتح رسالة ولي الأمر وتنزيل صورة التقرير. أرفق الصورة التي تم تنزيلها داخل محادثة واتساب.');return true;}
    notify('تم تنزيل صورة التقرير، لكن المتصفح منع فتح واتساب. اسمح بالنوافذ المنبثقة وأعد المحاولة؛ الصورة لم تُرسل.');return false;
  }catch(error){reserved.popup?.close();throw error;}
};
