...
function setupUnifiedHeader(){
  ...
  const links=[
    ['index.html','الرئيسية'],['materials.html','المحاضرات'],
...
function resourceCard(x,kind){
  ...
  const isTheory=kind==='material'&&(x.lectureCategory==='theory'||legacyClassLink),progress=Math.max(0,Math.min(100,Number(x.progress||0))),assignment=(currentStudentResources?.assignments||[]).find(row=>String(row.id)===String(x.linkedAssignmentId||'')),exam=(currentStudentResources?.exams||[]).find(row=>String(row.id)===String(x.linkedExamId||''));
  const actions=[fileUrl&&!isImage?`<a class="btn ghost" data-theory-open="${esc(x.id)}" target="_blank" rel="noopener noreferrer" href="${esc(fileUrl)}"><span data-icon="external-link"></span> فتح ملف الشرح</a>`:'',driveUrl?`<a class="btn primary class-link-button" data-theory-open="${esc(x.id)}" target="_blank" rel="noopener noreferrer" href="${esc(driveUrl)}"><span data-icon="external-link"></span> فتح رابط Google Drive</a>`:'',assignment?`<a class="btn ghost" data-student-destination="homework" href="student.html"><span data-icon="file-text"></span> حل ${esc(assignment.title||'الواجب')}</a>`:'',exam?`<a class="btn ghost" data-student-destination="exam" href="exams.html"><span data-icon="clipboard"></span> ${esc(exam.title||'الاختبار')}</a>`:'',isTheory?`<button class="btn ${progress>=100?'ghost':'primary'}" type="button" data-theory-complete="${esc(x.id)}" ${progress>=100?'disabled':''}><span data-icon="user-check"></span> ${progress>=100?'تم إكمال المحاضرة':'تحديد كمكتملة'}</button>`:''].filter(Boolean).join('');
  return `<article class="card resource-card student-track-resource ${driveUrl?'class-link-card':''} ${isTheory?'theory-learning-card':''}" data-resource-id="${esc(x.id||'')}"><div class="resource-top">${isTheory?`<span class="lecture-order-chip">${Number(x.lectureNumber||x.order||0)||'—'}</span>`:`<span class="iconbox" data-icon="${kind==='question'?'help-circle':'book-open'}"></span>`}<div class="resource-meta-badges"><span class="badge">${esc(x.grade||'كل المسارات')}</span>${meta.map(value=>`<span class="badge soft">${esc(value)}</span>`).join('')}${driveUrl?'<span class="badge good">رابط Drive</span>':''}${isTheory?`<span class="badge ${progress>=100?'good':progress>0?'warn':''}">${progress>=100?'مكتملة':progress>0?'قيد الدراسة':'جديدة'}</span>`:''}</div></div><h3>${esc(x.title||'بدون عنوان')}</h3><p>${esc(x.desc||x.content||(driveUrl?'رابط المحاضرة متاح على Google Drive.':' '))}</p>${isTheory?`<div class="theory-progress" aria-label="نسبة إنجاز المحاضرة"><span style="width:${progress}%"></span></div><small class="theory-progress-label">تم إنجاز ${progress}%</small>`:''}${preview}${actions?`<div class="resource-actions">${actions}</div>`:''}${kind==='question'&&x.answer?`<details class="resource-answer"><summary>عرض الإجابة النموذجية</summary><div class="written-box">${esc(x.answer)}</div></details>`:''}</article>`;
}
...
window.startExam=async function(examId,studentCode){
  ...
  next.addEventListener('click',()=>{saveVisibleAnswer();current++;renderCurrent();stage.scrollIntoView({behavior:'smooth',block:'start'});});
