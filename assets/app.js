...
function parentMonthlyReportText(report){
  ...
  lines.push('Techno Minds — تقرير ولي الأمر الشهري',`الطالب: ${st.name||'-'} — ${st.studentCode||'-'}`,`المسار: ${st.grade||'-'} — المجموعة: ${st.group||'-'}`,`الشهر: ${reportMonthLabel(report.monthKey)} — العام: ${st.academicYear||'-'}`,`آخر تحديث: ${formatPortalDate(report.generatedAt)} — Africa/Cairo`,
    `المستوى الأكاديمي: ${report.academicScore??'-'}% — ${report.academicLevel||'-'}`,`الالتزام: ${report.commitmentScore??'-'}% — ${report.commitmentLevel||'-'}`,`التقييم العام: ${report.overallScore??'-'}% — ${report.level||'-'}`,`المقارنة: ${report.trend?.label||'لا توجد بيانات كافية وقابلة للمقارنة'}`);
...
function parentReportWhatsAppIntro(report){
  ...
  return `السلام عليكم ورحمة الله وبركاته،\nمع حضرتك م. عمرو خالد، مهندس برمجيات ومدرس البرمجة والذكاء الاصطناعي ومؤسس Techno Minds.\n\nده التقرير الشهري الخاص بالطالب/ة ${st.name||'-'} عن شهر ${reportMonthLabel(report.monthKey)}، ومرفق لحضرتك صورة التقرير كاملة.\n\nحالة الطالب: ${report.level||'بيانات غير كافية'}${report.overallScore===null||report.overallScore===undefined?'':` — ${report.overallScore}%`}\nالحضور: ${attendance.present||0} من ${attendance.total||0} — الغياب: ${attendance.absent||0}\nالامتحانات التي تم تسليمها: ${results.submittedExams||0} — متوسط المصحح: ${results.average??'-'}%\nالواجبات: ${homework.submitted||0} من ${homework.required||0}\nالدفع: ${pay?(pay.status==='paid'?'تم الدفع بالكامل':pay.status==='partial'?`دفع جزئي — المتبقي ${formatPortalMoney(pay.remainingAmount)}`:'لم يتم الدفع'):'غير مسجل'}\nالتحفيز: ${report.motivation?`${report.motivation.totalPoints??0} نقطة`:'لا توجد حركة مسجلة'}\nالتقدم: ${report.trend?.label||'لا توجد بيانات كافية للمقارنة'}\n\nتقدر تتابع مستوى ابنك في أي وقت من الرابط ده:\n${portal}\n\nكود الطالب الموحّد: ${st.studentCode||'-'}\nاكتب الكود داخل صفحة ولي الأمر لعرض الحضور والواجبات والامتحانات والتقدم.\n\nمع تحيات م. عمرو خالد — Techno Minds`;
}
window.parentReportWhatsAppIntro=parentReportWhatsAppIntro;

async function parentReportImageBlob(report){
  await document.fonts?.ready;
  const canvas=document.createElement('canvas'),width=1080,margin=64,ctx=canvas.getContext('2d'),headings=new Set(['الحضور','الامتحانات والدرجات','الواجبات','التطبيق والمحاضرات','التحفيز الشهري','دفع الشهر','حدود البيانات','نقاط القوة','يحتاج متابعة','خطوات المتابعة','ملاحظات م. عمرو خالد']);
  ctx.font='27px Cairo, Arial, sans-serif';const entries=[];
  for(const paragraph of parentMonthlyReportText(report).split('\n')){if(paragraph.startsWith('Techno Minds —'))continue;const heading=headings.has(paragraph),maxWidth=width-2*margin-(heading?28:0);if(!paragraph){entries.push({text:'',kind:'gap'});continue;}let line='';for(const word of paragraph.split(' ')){const next=line?`${line} ${word}`:word;if(line&&ctx.measureText(next).width>maxWidth){entries.push({text:line,kind:heading?'heading':'body'});line=word;}else line=next;}if(line)entries.push({text:line,kind:heading?'heading':'body'});}
  const height=260+entries.reduce((sum,item)=>sum+(item.kind==='heading'?72:item.kind==='gap'?18:46),0)+90;
  if(height>24000)throw new Error('التقرير طويل للصورة؛ استخدم طباعة / حفظ PDF للاحتفاظ بكل البيانات.');
  canvas.width=width;canvas.height=height;ctx.direction='rtl';ctx.textAlign='right';ctx.fillStyle='#f3f6fb';ctx.fillRect(0,0,width,height);ctx.fillStyle='#071b36';ctx.fillRect(0,0,width,210);ctx.fillStyle='#d9ad4f';ctx.fillRect(0,200,width,10);ctx.font='bold 42px Cairo, Arial, sans-serif';ctx.fillStyle='#fff';ctx.fillText('Techno Minds — تقرير ولي الأمر الشهري',width-margin,72);ctx.font='bold 34px Cairo, Arial, sans-serif';ctx.fillStyle='#f5d48d';ctx.fillText(report?.student?.name||'-',width-margin,128);ctx.font='24px Cairo, Arial, sans-serif';ctx.fillStyle='#d8e2ef';ctx.fillText(`${reportMonthLabel(report.monthKey)} · ${report?.student?.grade||'-'} · ${report?.student?.group||'-'}`,width-margin,172);
  let y=246;for(const item of entries){if(item.kind==='gap'){y+=18;continue;}if(item.kind==='heading'){ctx.fillStyle='#e8eef7';ctx.fillRect(margin,y-37,width-2*margin,56);ctx.fillStyle='#b78a2d';ctx.fillRect(width-margin-10,y-37,10,56);ctx.font='bold 29px Cairo, Arial, sans-serif';ctx.fillStyle='#071b36';ctx.fillText(item.text,width-margin-28,y);y+=72;continue;}ctx.font='27px Cairo, Arial, sans-serif';ctx.fillStyle='#22334d';ctx.fillText(item.text,width-margin,y);y+=46;}
  ctx.fillStyle='#071b36';ctx.fillRect(0,height-72,width,72);ctx.font='bold 24px Cairo, Arial, sans-serif';ctx.fillStyle='#f5d48d';ctx.fillText('مع تحيات م. عمرو خالد — Techno Minds',width-margin,height-27);
