(function(){
  'use strict';
  const safe=value=>String(value??'').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));
  const sections=[
    ['lectures','هيكل المحاضرات'],['lecture_materials','ملفات المحاضرات'],['question_banks','بنوك الأسئلة'],
    ['bank_questions','أسئلة البنك'],['teacher_files','ملفات المدرس']
  ];
  let active='lectures',rows=[],cursor=null,hasMore=false,editing=null;

  const gradeOptions='<option value="">اختر الصف</option><option>أولى ثانوي بكالوريا</option><option>تانية ثانوي بكالوريا</option><option>أساسيات برمجة</option><option>مبتدئين برمجة</option>';
  const termOptions='<option value="">اختر الترم</option><option>الترم الأول</option><option>الترم الثاني</option>';
  const groupsForGrade=grade=>(typeof adminData!=='undefined'&&Array.isArray(adminData.groups)?adminData.groups:[]).filter(group=>group.active!==false&&(!grade||typeof window.adminSameAcademic!=='function'||window.adminSameAcademic(group.grade,grade)));
  const groupOptions=(grade,selected='')=>`<option value="">كل المجموعات</option>${groupsForGrade(grade).map(group=>`<option value="${safe(group.name||group.group||'')}" data-schedule-id="${safe(group.id||group.scheduleId||'')}" ${String(group.name||group.group||'')===String(selected)?'selected':''}>${safe(group.name||group.group||'')}</option>`).join('')}`;

  function shell(){
    return `<div class="section-head"><div><span class="kicker">مكتبة المنهج</span><h2>المنهج وبنك الأسئلة</h2><p class="section-desc">نظّم هيكل المنهج وملفات المحاضرات والأسئلة. التسليم والتصحيح يتمان من قسمي الواجبات والاختبارات.</p></div><button class="btn primary" type="button" data-new-content>إضافة محتوى</button></div>
    <div class="curriculum-admin-tabs">${sections.map(item=>`<button type="button" class="${item[0]===active?'active':''}" data-content-section="${item[0]}">${item[1]} <small data-section-count="${item[0]}"></small></button>`).join('')}</div>
    <div class="card curriculum-admin-toolbar"><input type="search" id="curriculumSearch" placeholder="بحث بالعنوان" aria-label="بحث بالعنوان"><select id="curriculumGradeFilter" aria-label="فلترة حسب الصف">${gradeOptions}<option value="all">كل الصفوف</option></select><select id="curriculumTermFilter" aria-label="فلترة حسب الترم">${termOptions}<option value="all">كل الترمات</option></select><select id="curriculumDirection" aria-label="الترتيب"><option value="asc">تصاعدي</option><option value="desc">تنازلي</option></select><button class="btn ghost" type="button" data-apply-filter>تطبيق</button></div>
    <div id="curriculumAdminList"><div class="skeleton" style="height:240px"></div></div>
    <div class="pagination-actions"><button class="btn ghost" type="button" data-load-more hidden>تحميل المزيد</button></div>
    <section class="card curriculum-import"><h3>استيراد ملفات المنهج</h3><p>ارفع عدة ملفات PDF أو Word أو صور بحد أقصى 15MB للملف. يتم استخراج رقم المحاضرة من اسم الملف ويمكن مراجعة البيانات قبل الحفظ.</p><div class="grid three"><select id="importGrade">${gradeOptions}</select><select id="importTerm">${termOptions}</select><input id="importUnit" placeholder="اسم/معرّف الوحدة"></div><label class="btn ghost file-button">اختيار الملفات<input id="curriculumBulkFiles" type="file" multiple accept=".pdf,.doc,.docx,image/jpeg,image/png,image/webp" hidden></label><div id="curriculumImportPreview"></div></section>
    <details class="card curriculum-migration curriculum-maintenance"><summary><span><b>أدوات الصيانة والترحيل</b><small>استخدمها فقط عند استيراد أو ترحيل محتوى قديم.</small></span></summary><div class="admin-detail-body"><p>الفحص التجريبي لا يكتب أو يحذف أي بيانات. التنفيذ قابل للإعادة ويمنع التكرار.</p><button class="btn ghost" type="button" data-migration-dry>فحص Dry Run</button><button class="btn primary" type="button" data-migration-apply>تنفيذ الترحيل</button><pre id="curriculumMigrationResult" aria-live="polite"></pre></div></details>`;
  }

  function list(){
    const query=(document.getElementById('curriculumSearch')?.value||'').trim().toLowerCase();
    const filtered=rows.filter(item=>!query||String(item.title||'').toLowerCase().includes(query));
    const box=document.getElementById('curriculumAdminList');if(!box)return;
    box.innerHTML=filtered.length?`<div class="curriculum-admin-list">${filtered.map(item=>`<article class="curriculum-admin-row"><div><small>${safe(item.grade||'')} · ${safe(item.term||'')} · رقم ${Number(item.lectureNumber||item.order||0)}</small><h3>${safe(item.title)}</h3><p>${safe(item.description||'')}</p></div><span class="badge ${item.status==='published'?'good':item.status==='hidden'?'danger':'warn'}">${item.status==='published'?'منشور':item.status==='hidden'?'مخفي':'مسودة'}</span><div class="mobile-actions"><button class="small-btn" type="button" data-edit-content="${safe(item.id)}">تعديل</button><button class="small-btn" type="button" data-copy-content="${safe(item.id)}">نسخ</button><button class="small-btn danger" type="button" data-delete-content="${safe(item.id)}">حذف</button></div></article>`).join('')}</div>`:'<div class="empty-state"><h3>لا يوجد محتوى مطابق</h3><p>ابدأ بإضافة أول عنصر أو غيّر الفلاتر.</p></div>';
    bindRows();
  }

  async function load(append=false){
    const box=document.getElementById('curriculumAdminList');if(!append&&box)box.innerHTML='<div class="skeleton" style="height:240px"></div>';
    try{
      const grade=document.getElementById('curriculumGradeFilter')?.value;
      const term=document.getElementById('curriculumTermFilter')?.value;
      const result=await window.MFCloud.listCurriculumAdmin({collection:active,pageSize:20,cursor:append?cursor:null,direction:document.getElementById('curriculumDirection')?.value||'asc',grade:grade&&grade!=='all'?grade:'',term:term&&term!=='all'?term:''});
      rows=append?[...rows,...result.rows]:result.rows;cursor=result.nextCursor;hasMore=result.hasMore;list();
      const more=document.querySelector('[data-load-more]');if(more)more.hidden=!hasMore;
    }catch(error){if(box)box.innerHTML=`<div class="empty-state"><h3>تعذر تحميل المحتوى</h3><p>${safe(error?.message||'انشر Functions والفهارس الجديدة أولًا.')}</p></div>`;}
  }

  function modal(item={}){
    editing=item.id||null;
    document.getElementById('curriculumEditorModal')?.remove();
    const isQuestion=['bank_questions','assignment_questions','exam_questions_v2'].includes(active);
    document.body.insertAdjacentHTML('beforeend',`<div class="curriculum-modal" id="curriculumEditorModal" role="dialog" aria-modal="true" aria-labelledby="curriculumEditorTitle"><form class="card curriculum-editor" id="curriculumEditorForm"><button class="curriculum-modal-close" type="button" aria-label="إغلاق">×</button><h2 id="curriculumEditorTitle">${editing?'تعديل':'إضافة'} ${safe(sections.find(section=>section[0]===active)?.[1]||'محتوى')}</h2>
      <div class="grid three"><label class="field"><span>الصف</span><select name="grade" required>${gradeOptions}</select></label><label class="field"><span>الترم</span><select name="term" required>${termOptions}</select></label><label class="field"><span>المجموعة</span><select name="group">${groupOptions(item.grade,item.group)}</select></label></div><label class="field"><span>الوحدة</span><input name="unitId" required></label>
      <div class="grid three"><label class="field"><span>رقم المحاضرة</span><input name="lectureNumber" type="number" min="1" max="36" required></label><label class="field"><span>معرّف المحاضرة</span><input name="lectureId"></label><label class="field"><span>الترتيب</span><input name="order" type="number" min="0"></label></div>
      <label class="field"><span>العنوان</span><input name="title" maxlength="220" required></label><label class="field"><span>الوصف</span><textarea name="description" rows="3"></textarea></label><label class="field"><span>أهداف التعلم — هدف في كل سطر</span><textarea name="learningObjectives" rows="4"></textarea></label>
      ${isQuestion?`<div class="grid three"><label class="field"><span>نوع السؤال</span><select name="questionType"><option value="mcq">اختيار من متعدد</option><option value="true_false">صح وخطأ</option><option value="complete">أكمل</option><option value="concept">مفاهيم</option><option value="trace_code">توقع ناتج الكود</option><option value="debug">اكتشاف الخطأ</option><option value="order_code">ترتيب الكود</option><option value="program">كتابة برنامج</option><option value="essay">مقالي</option></select></label><label class="field"><span>الصعوبة</span><select name="difficulty"><option>سهل</option><option>متوسط</option><option>متقدم</option></select></label><label class="field"><span>الدرجة</span><input name="points" type="number" min="0"></label></div><label class="field"><span>الاختيارات — اختيار في كل سطر</span><textarea name="choices"></textarea></label><label class="field"><span>الإجابة الصحيحة</span><textarea name="correctAnswer"></textarea></label><label class="field"><span>شرح الإجابة</span><textarea name="answerExplanation"></textarea></label>`:''}
      <div class="grid three"><label class="field"><span>تاريخ الفتح</span><input name="openAt" type="datetime-local"></label><label class="field"><span>تاريخ الإغلاق</span><input name="closeAt" type="datetime-local"></label><label class="field"><span>الحالة</span><select name="status"><option value="draft">مسودة</option><option value="published">منشور</option><option value="hidden">مخفي</option></select></label></div>
      <label class="field"><span>ملف اختياري (PDF / Word / صورة، 15MB)</span><input name="file" type="file" accept=".pdf,.doc,.docx,image/jpeg,image/png,image/webp"></label><label class="check"><input name="allowDownload" type="checkbox"> السماح بالتحميل</label>
      <div class="mobile-actions"><button class="btn primary" type="submit">حفظ</button><button class="btn ghost" type="button" data-cancel-editor>إلغاء</button></div></form></div>`);
    const form=document.getElementById('curriculumEditorForm');
    Object.entries(item).forEach(([key,value])=>{const field=form.elements[key];if(!field)return;if(field.type==='checkbox')field.checked=value===true;else if(key==='learningObjectives'&&Array.isArray(value))field.value=value.join('\n');else if(key==='choices'&&Array.isArray(value))field.value=value.join('\n');else if(field.type==='datetime-local')field.value=typeof isoToLocalDateTimeInput==='function'?isoToLocalDateTimeInput(value?.toDate?value.toDate():value):String(value||'').slice(0,16);else if(!value?.toDate)field.value=value??'';});
    form.elements.grade.onchange=()=>{const current=form.elements.group.value;form.elements.group.innerHTML=groupOptions(form.elements.grade.value,current);};
    form.querySelector('.curriculum-modal-close').onclick=()=>form.closest('.curriculum-modal').remove();form.querySelector('[data-cancel-editor]').onclick=()=>form.closest('.curriculum-modal').remove();form.onsubmit=save;
  }

  async function save(event){
    event.preventDefault();const form=event.currentTarget,button=form.querySelector('[type="submit"]');button.disabled=true;
    try{
      const data=Object.fromEntries(new FormData(form).entries()),file=form.elements.file.files[0];delete data.file;
      data.scheduleId=data.groupId=String(form.elements.group?.selectedOptions?.[0]?.dataset?.scheduleId||'');
      data.openAt=typeof localDateTimeToIso==='function'?localDateTimeToIso(data.openAt):data.openAt;data.closeAt=typeof localDateTimeToIso==='function'?localDateTimeToIso(data.closeAt):data.closeAt;
      data.allowDownload=form.elements.allowDownload.checked;data.learningObjectives=String(data.learningObjectives||'').split('\n').filter(Boolean);if(data.choices)data.choices=String(data.choices).split('\n').filter(Boolean);
      if(file){if(file.size>15*1024*1024)throw new Error('حجم الملف أكبر من 15MB.');const upload=await window.MFCloud.uploadAttachment(file,active==='teacher_files'?'teacher-files':`curriculum/${data.grade}/${active}`);data.filePath=upload.path;data.fileName=file.name;data.contentType=file.type;}
      await window.MFCloud.upsertCurriculumEntity({collection:active,id:editing||undefined,data});form.closest('.curriculum-modal').remove();if(typeof aToast==='function')aToast('تم حفظ المحتوى');await load();
    }catch(error){if(typeof aToast==='function')aToast(error?.message||'تعذر حفظ المحتوى');}finally{button.disabled=false;}
  }

  function confirmDelete(item){
    document.getElementById('curriculumConfirmModal')?.remove();document.body.insertAdjacentHTML('beforeend',`<div class="curriculum-modal" id="curriculumConfirmModal" role="dialog" aria-modal="true"><div class="card confirm-card"><h2>حذف المحتوى؟</h2><p>سيتم حذف «${safe(item.title)}». لا يؤثر ذلك في البيانات القديمة.</p><div class="mobile-actions"><button class="btn danger" type="button" data-confirm-delete>حذف</button><button class="btn ghost" type="button" data-cancel-delete>إلغاء</button></div></div></div>`);const modal=document.getElementById('curriculumConfirmModal');modal.querySelector('[data-cancel-delete]').onclick=()=>modal.remove();modal.querySelector('[data-confirm-delete]').onclick=async event=>{event.currentTarget.disabled=true;await window.MFCloud.deleteCurriculumEntity(active,item.id);modal.remove();await load();};
  }

  function bindRows(){
    document.querySelectorAll('[data-edit-content]').forEach(button=>button.onclick=()=>modal(rows.find(item=>item.id===button.dataset.editContent)||{}));
    document.querySelectorAll('[data-copy-content]').forEach(button=>button.onclick=()=>{const item={...(rows.find(row=>row.id===button.dataset.copyContent)||{})};delete item.id;item.title=`نسخة من ${item.title||''}`;item.status='draft';modal(item);});
    document.querySelectorAll('[data-delete-content]').forEach(button=>button.onclick=()=>confirmDelete(rows.find(item=>item.id===button.dataset.deleteContent)||{}));
  }

  function previewFiles(files){
    const box=document.getElementById('curriculumImportPreview');if(!box)return;const list=[...files];box.innerHTML=list.map((file,index)=>{const number=Number(file.name.match(/(?:lecture|lec|محاضرة|l)[-_ ]?(\d{1,2})/i)?.[1]||file.name.match(/\b(\d{1,2})\b/)?.[1]||0);return `<div class="import-preview-row" data-import-index="${index}"><span>${safe(file.name)}</span><input value="${safe(file.name.replace(/\.[^.]+$/,''))}" aria-label="العنوان"><input type="number" min="1" max="36" value="${number||''}" placeholder="رقم المحاضرة" aria-label="رقم المحاضرة"><button class="small-btn primary" type="button">رفع</button><small>${(file.size/1024/1024).toFixed(2)} MB</small></div>`;}).join('');box.querySelectorAll('.import-preview-row').forEach(row=>row.querySelector('button').onclick=async()=>{const file=list[Number(row.dataset.importIndex)],button=row.querySelector('button');if(file.size>15*1024*1024){button.textContent='أكبر من 15MB';button.classList.add('danger');return;}const grade=document.getElementById('importGrade').value,term=document.getElementById('importTerm').value,unitId=document.getElementById('importUnit').value,lectureNumber=row.querySelector('input[type="number"]').value,title=row.querySelector('input:not([type="number"])').value;if(!grade||!term||!unitId||!lectureNumber){if(typeof aToast==='function')aToast('أكمل الصف والترم والوحدة ورقم المحاضرة');return;}button.disabled=true;button.textContent='جارٍ الرفع';try{const uploaded=await window.MFCloud.uploadAttachment(file,`curriculum/${grade}/${active}`);await window.MFCloud.upsertCurriculumEntity({collection:active,data:{grade,term,unitId,lectureId:`lecture-${lectureNumber}`,lectureNumber,order:lectureNumber,title,status:'draft',filePath:uploaded.path,fileName:file.name,contentType:file.type}});button.textContent='نجح';button.classList.add('good');}catch(error){button.textContent='فشل';button.classList.add('danger');}finally{button.disabled=false;}});}

  async function migration(apply){const output=document.getElementById('curriculumMigrationResult');output.textContent='جاري الفحص...';try{const result=await window.MFCloud.migrateCurriculumV61(apply);output.textContent=JSON.stringify(result,null,2);}catch(error){output.textContent=error?.message||'تعذر تشغيل الترحيل';}}

  window.renderCurriculumAdmin=function(){
    if(typeof content!=='function')return;content(shell());
    document.querySelectorAll('[data-content-section]').forEach(button=>button.onclick=()=>{active=button.dataset.contentSection;rows=[];cursor=null;window.renderCurriculumAdmin();});
    document.querySelector('[data-new-content]').onclick=()=>modal();document.querySelector('[data-apply-filter]').onclick=()=>load();document.getElementById('curriculumSearch').oninput=list;document.querySelector('[data-load-more]').onclick=()=>load(true);document.getElementById('curriculumBulkFiles').onchange=event=>previewFiles(event.target.files);document.querySelector('[data-migration-dry]').onclick=()=>migration(false);document.querySelector('[data-migration-apply]').onclick=()=>migration(true);load();
  };

  let questionBankRows=[], questionBankCursor=null, questionBankRequest=0, questionBankLecture='';
  const theoryLessons=()=> (typeof adminData!=='undefined'?adminData.materials||[]:[]).filter(row=>row.archived!==true&&['theory','class-link','theory-lecture'].some(kind=>[row.lectureCategory,row.materialType,row.resourceType].includes(kind)));
  function bankLessonOptions(selected='',includeGeneral=true){
    const lessons=theoryLessons();
    return `${includeGeneral?'<option value="">بنك عام — بدون درس محدد</option>':'<option value="">اختر الدرس</option>'}${lessons.map(row=>`<option value="${safe(row.id)}" ${String(row.id)===String(selected)?'selected':''}>${safe(row.title||row.lecture||row.id)} · ${safe(row.grade||'')} ${row.group?'· '+safe(row.group):''}</option>`).join('')}`;
  }
  function questionBankCard(item){
    const published=item.active!==false&&item.published===true&&item.status==='published';
    return `<article class="card question-bank-admin-card"><div><span class="badge ${published?'good':'warn'}">${published?'منشور للطلاب':'غير منشور'}</span><h3>${safe(item.title||'بنك أسئلة')}</h3><p>${safe(item.description||'')}</p><small>${safe(item.lectureTitle||'بنك عام')} · ${safe(item.grade||'')} · ${safe(item.group||'كل المجموعات')}</small>${item.filePath?`<p>PDF: ${safe(item.fileName||'مرفق')}</p>`:''}</div><div class="mobile-actions"><button class="small-btn" type="button" data-edit-question-bank="${safe(item.id)}">تعديل</button><button class="small-btn danger" type="button" data-archive-question-bank="${safe(item.id)}">أرشفة</button></div></article>`;
  }
  function paintQuestionBanks(){
    const box=document.getElementById('questionBanksAdminList');if(!box)return;
    box.innerHTML=questionBankRows.length?questionBankRows.map(questionBankCard).join(''):'<div class="empty-state"><h3>لا توجد بنوك أسئلة في هذه الصفحة</h3><p>أضف PDF للدرس، أو حمّل الصفحة التالية إن توفرت.</p></div>';
    box.querySelectorAll('[data-edit-question-bank]').forEach(button=>button.onclick=()=>openQuestionBankEditor(questionBankRows.find(item=>String(item.id)===button.dataset.editQuestionBank)||{}));
    box.querySelectorAll('[data-archive-question-bank]').forEach(button=>button.onclick=async()=>{const item=questionBankRows.find(row=>String(row.id)===button.dataset.archiveQuestionBank);if(!item||!confirm(`أرشفة «${item.title}» وإخفاؤه عن الطلاب؟`))return;button.disabled=true;try{await window.MFCloud.deleteCurriculumEntity('question_banks',item.id);aToast('تمت أرشفة بنك الأسئلة');await loadQuestionBanks();}catch(error){aToast(error?.message||'تعذرت الأرشفة');}finally{button.disabled=false;}});
  }
  async function loadQuestionBanks(append=false){
    const box=document.getElementById('questionBanksAdminList'),more=document.getElementById('questionBanksLoadMore');if(!box)return;
    const request=++questionBankRequest,lectureId=questionBankLecture;
    if(more)more.disabled=true;
    if(!append){questionBankRows=[];questionBankCursor=null;box.innerHTML='<div class="portal-loading" role="status">جارٍ تحميل بنك الأسئلة…</div>';}
    try{
      const result=await window.MFCloud.listCurriculumAdmin({collection:'question_banks',pageSize:30,direction:'desc',lectureId,cursor:append?questionBankCursor:null});
      if(request!==questionBankRequest||!box.isConnected)return;
      questionBankRows=[...questionBankRows,...(result.rows||[]).filter(item=>item.archived!==true)];questionBankCursor=result.nextCursor;
      paintQuestionBanks();if(more)more.hidden=!result.hasMore;
    }catch(error){if(request===questionBankRequest&&box.isConnected){box.innerHTML=`<div class="empty-state"><h3>تعذر تحميل بنك الأسئلة</h3><p>${safe(error?.message||'حاول مرة أخرى.')}</p><button class="btn ghost" type="button" data-bank-retry>إعادة المحاولة</button></div>`;box.querySelector('[data-bank-retry]').onclick=()=>loadQuestionBanks(append);}}
    finally{if(request===questionBankRequest&&more)more.disabled=false;}
  }
  function bankWorkspace(embedded){
    return `<section class="${embedded?'card theory-question-banks':''}" id="theoryQuestionBanks"><div class="section-head"><div><h2 class="section-title">بنك الأسئلة</h2><p class="section-desc">ملفات PDF وأسئلة لكل درس. يرث البنك مسار ومجموعة الدرس المرتبط به.</p></div></div><div class="question-bank-toolbar"><label><span>الدرس النظري</span><select id="questionBankLessonFilter">${bankLessonOptions('',!embedded)}</select></label><button class="btn primary" type="button" id="addQuestionBankButton">+ إضافة بنك أسئلة</button></div><div class="question-bank-admin-list" id="questionBanksAdminList"></div><button class="btn ghost question-bank-pagination" id="questionBanksLoadMore" type="button" hidden>تحميل المزيد</button></section>`;
  }
  function bindBankWorkspace(embedded){
    questionBankLecture='';questionBankRequest++;
    const select=document.getElementById('questionBankLessonFilter'),add=document.getElementById('addQuestionBankButton');
    const change=()=>{questionBankLecture=select.value;add.disabled=embedded&&!select.value;if(embedded&&!select.value){questionBankRequest++;document.getElementById('questionBanksLoadMore').hidden=true;document.getElementById('questionBanksAdminList').innerHTML='<p class="section-desc">اختر الدرس لعرض ملفات بنك الأسئلة أو رفع PDF جديد له.</p>';return;}loadQuestionBanks();};
    select.onchange=change;add.onclick=()=>openQuestionBankEditor({lectureId:questionBankLecture,lectureSource:questionBankLecture?'materials':''});document.getElementById('questionBanksLoadMore').onclick=()=>loadQuestionBanks(true);change();
  }
  window.mountTheoryQuestionBanks=function(){const target=document.getElementById('theoryBanksMount');if(!target)return;target.innerHTML=bankWorkspace(true);bindBankWorkspace(true);};
  window.showTheoryQuestionBanks=function(id){const select=document.getElementById('questionBankLessonFilter');if(!select)return;select.value=id;select.onchange();document.getElementById('theoryQuestionBanks').scrollIntoView({block:'start',behavior:window.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});select.focus({preventScroll:true});};

  function openQuestionBankEditor(item={}){
    if(document.getElementById('questionBankEditorModal'))return;
    const returnFocus=document.activeElement,ctx=window.adminWorkspaceContext?.()||{academicYear:'',term:'الترم الأول'},id=item.id||crypto.randomUUID();
    const linked=theoryLessons().find(row=>String(row.id)===String(item.lectureId)&&item.lectureSource==='materials');
    const oldLink=item.lectureId&&!linked?`<option value="${safe(item.lectureId)}" selected>${safe(item.lectureTitle||'الدرس المرتبط الحالي')}</option>`:'';
    document.body.insertAdjacentHTML('beforeend',`<div class="curriculum-modal" id="questionBankEditorModal" role="dialog" aria-modal="true" aria-labelledby="questionBankEditorTitle"><form class="card curriculum-editor question-bank-editor" id="questionBankEditorForm" data-allow-rapid-submit="true"><button class="curriculum-modal-close" type="button" aria-label="إغلاق">×</button><h2 id="questionBankEditorTitle">${item.id?'تعديل بنك الأسئلة':'إضافة بنك أسئلة'}</h2><label class="field"><span>الدرس النظري المرتبط</span><select name="lectureId">${bankLessonOptions(item.lectureId)}${oldLink}</select></label><div class="grid three"><label class="field"><span>المسار</span><select name="grade" required>${gradeOptions}</select></label><label class="field"><span>الترم</span><select name="term">${termOptions}</select></label><label class="field"><span>المجموعة</span><select name="group">${groupOptions(item.grade,item.group)}</select></label></div><label class="field"><span>عنوان بنك الأسئلة</span><input name="title" maxlength="220" required placeholder="مثال: أسئلة درس الشروط"></label><label class="field"><span>وصف مختصر</span><textarea name="description" rows="2" maxlength="4000"></textarea></label><label class="field"><span>أسئلة مكتوبة (اختياري مع PDF)</span><textarea name="content" rows="5" maxlength="50000"></textarea></label><label class="field question-bank-pdf-field"><span>ملف PDF — حتى 15MB</span><input name="file" type="file" accept="application/pdf,.pdf">${item.filePath?`<small>الملف الحالي: ${safe(item.fileName||'PDF')} — اترك الحقل فارغًا للاحتفاظ به.</small>`:''}</label><div class="grid two"><label class="field"><span>الظهور</span><select name="status"><option value="published">منشور للطلاب</option><option value="draft">مسودة</option><option value="hidden">مخفي</option></select></label><label class="check"><input name="allowDownload" type="checkbox"> إتاحة التنزيل للطالب</label></div><div class="question-bank-upload-state" role="status" aria-live="polite"><span data-bank-state></span><progress max="100" value="0" hidden aria-label="تقدم رفع PDF"></progress></div><div class="mobile-actions"><button class="btn primary" type="submit">حفظ بنك الأسئلة</button><button class="btn ghost" type="button" data-close-question-bank>إلغاء</button></div></form></div>`);
    const modal=document.getElementById('questionBankEditorModal'),form=document.getElementById('questionBankEditorForm'),state=form.querySelector('[data-bank-state]'),progress=form.querySelector('progress');
    let busy=false,uploaded=null,selectedFile=null;
    ['grade','term','group','title','description','content','status'].forEach(name=>{if(item[name]!==undefined)form.elements[name].value=item[name]||'';});
    if(!form.elements.term.value)form.elements.term.value=item.term||ctx.term;
    form.elements.allowDownload.checked=item.allowDownload===true;
    const updateTarget=()=>{const lesson=theoryLessons().find(row=>String(row.id)===form.elements.lectureId.value);if(lesson){form.elements.grade.value=lesson.grade;form.elements.term.value=lesson.term||ctx.term;form.elements.group.innerHTML=groupOptions(lesson.grade,lesson.group);form.elements.group.value=lesson.group||'';}['grade','term','group'].forEach(name=>form.elements[name].disabled=Boolean(form.elements.lectureId.value));};
    form.elements.lectureId.onchange=updateTarget;form.elements.grade.onchange=()=>{form.elements.group.innerHTML=groupOptions(form.elements.grade.value);};updateTarget();
    const close=()=>{if(busy){state.textContent='انتظر تأكيد الحفظ قبل الإغلاق.';return;}modal.remove();document.documentElement.classList.remove('question-bank-dialog-open');if(returnFocus?.isConnected)returnFocus.focus({preventScroll:true});};
    modal.querySelector('.curriculum-modal-close').onclick=close;form.querySelector('[data-close-question-bank]').onclick=close;
    modal.addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();close();}if(event.key==='Tab'){const controls=[...form.querySelectorAll('button,input,select,textarea,a[href]')].filter(el=>!el.disabled&&!el.hidden),first=controls[0],last=controls.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}}});
    document.documentElement.classList.add('question-bank-dialog-open');form.elements.title.focus();
    form.onsubmit=async event=>{
      event.preventDefault();if(busy)return;
      const button=form.querySelector('[type="submit"]'),file=form.elements.file.files[0],values=Object.fromEntries(new FormData(form).entries());delete values.file;
      for(const name of ['grade','term','group'])values[name]=form.elements[name].value;
      values.lectureSource=values.lectureId?(theoryLessons().some(row=>String(row.id)===values.lectureId)?'materials':item.lectureSource||'lectures'):'';
      if(!String(values.content||'').trim()&&!file&&!item.filePath){state.textContent='اكتب أسئلة أو اختر ملف PDF أولًا.';return;}
      busy=true;button.disabled=true;button.classList.add('is-loading');state.textContent='جارٍ التحقق من الملف…';
      try{
        if(file){
          if(!file.size||file.size>15*1024*1024)throw new Error('اختر ملفًا غير فارغ حتى 15MB.');
          if(!/\.pdf$/i.test(file.name))throw new Error('اختر ملف PDF صحيحًا.');
          const signature=await file.slice(0,5).text();if(signature!=='%PDF-')throw new Error('محتوى الملف ليس PDF صحيحًا.');
          if(file!==selectedFile){uploaded=null;selectedFile=file;}
          if(!uploaded){progress.hidden=false;progress.value=0;const normalized=new File([file],file.name,{type:'application/pdf'});uploaded=await window.MFCloud.uploadAttachment(normalized,`curriculum/${values.grade}/question_banks`,{private:true,fileName:`${crypto.randomUUID()}.pdf`,onProgress:update=>{progress.value=update.percent;state.textContent=`جارٍ رفع PDF: ${update.percent}%`;}});}
          values.filePath=uploaded.path;values.fileName=file.name;values.contentType='application/pdf';
        }else{values.filePath=item.filePath||'';values.fileName=item.fileName||'';values.contentType=item.contentType||'';}
        values.scheduleId=values.groupId=String(form.elements.group.selectedOptions?.[0]?.dataset?.scheduleId||'');values.academicYear=item.academicYear||ctx.academicYear||'';values.allowDownload=form.elements.allowDownload.checked;values.active=values.status==='published';values.order=Number(item.order||0);
        state.textContent='اكتمل الرفع؛ جارٍ تأكيد حفظ بنك الأسئلة…';await window.MFCloud.upsertCurriculumEntity({collection:'question_banks',id,data:values});
        busy=false;close();aToast(values.status==='published'?'تم حفظ بنك الأسئلة ونشره للطلاب المسموح لهم':'تم حفظ بنك الأسئلة');await loadQuestionBanks();
      }catch(error){state.textContent=error?.message||'تعذر الحفظ. يمكنك إعادة المحاولة دون إعادة رفع نفس الملف.';}
      finally{busy=false;button.disabled=false;button.classList.remove('is-loading');}
    };
  }
  window.renderQuestionBanksAdmin=function(){if(typeof content!=='function')return;content(bankWorkspace(false));bindBankWorkspace(false);hydrateIcons();};
})();
